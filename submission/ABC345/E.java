...
final class Main{
	...
	public static void main(String[] args){
		...
		int N = sc.nextInt();
		int K = sc.nextInt();
		ArrayList<int[]> ball = new ArrayList<>();
		ball.add(sc.nextInt(2));
		for(int i=1;i<N;i++){
			int[] temp = sc.nextInt(2);
			if(ball.get(ball.size()-1)[0]==temp[0]){
				ball.get(ball.size()-1)[1] = Math.max(ball.get(ball.size()-1)[1],temp[1]);
				K--;
			}
			else
				ball.add(temp);
		...
		ArrayList<HashMap<Integer,Long>> map = new ArrayList<>(K<<1);
		for(int i=0;i<=K;i++)
			map.add(new HashMap<>());
		map.get(0).put(0,0L);
		long sum = 0;
		...
		for(int[] b:ball){
			sum += b[1];
			for(Entry<Integer,Long> entry:map.get(K).entrySet())
				if(entry.getKey()!=b[0])
					min = Math.min(entry.getValue(),min);
			map.set(K,new HashMap<>());
			Integer bb = b[0];
			for(int i=K-1;i>=0;i--){
				HashMap<Integer,Long> next = new HashMap<>();
				for(Entry<Integer,Long> entry:map.get(i).entrySet()){
					if(entry.getKey()!=b[0])
						next.merge(bb,entry.getValue(),Long::min);
					map.get(i+1).merge(entry.getKey(),(long)entry.getValue()+b[1],Long::min);
				}
				map.set(i,next);
			}
		}
		for(long num:map.get(K).values())
