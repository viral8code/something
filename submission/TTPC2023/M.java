...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		int[] primes = MathFunction.primes(30);

		class Data{
			long P;
			int[] count;
			boolean isInteger;
			public Data(long p,int[] c){
				P = p;
				count = c;
				isInteger = true;
			...
			public Data add(Data d){
				...
				long qa = 1;
				for(int i=0;i<primes.length;i++)
					qa *= MathFunction.pow(primes[i],Math.max(0,count[i]));
				pa %= qa;
				long pb = d.P;
				long qb = 1;
				for(int i=0;i<primes.length;i++)
					qb *= MathFunction.pow(primes[i],Math.max(0,d.count[i]));
				pb %= qb;
				long sum = qb*pa+pb*qa;
				int[] next = new int[primes.length];
				for(int i=0;i<primes.length;i++){
					next[i] = count[i]+d.count[i];
					while(next[i]>0&&sum%primes[i]==0){
						next[i]--;
						sum /= primes[i];
					}
				}
				return new Data(sum,next);
			}
			public int hashCode(){
				return (int)P^Arrays.hashCode(count);
			}
			public boolean equals(Object o){
				if(o instanceof Data d){
					if(P!=d.P)
						return false;
					return Arrays.equals(count,d.count);
				}
				return false;
			}
			public String toString(){
				return P+":"+Arrays.toString(count);
			}
		}

		int N = sc.nextInt();
		HashMap<Data,Long> map = new HashMap<>();
		Data zero = new Data(0,new int[primes.length]);
		long ans = 0;
		while(N-->0){
			int p = sc.nextInt();
			int q = sc.nextInt();
			...
			Data now = new Data(subP,count);
			HashMap<Data,Long> next = new HashMap<>();
			long sum = 0;
			for(Map.Entry<Data,Long> entry:map.entrySet()){
				Data nextN = entry.getKey().add(now);
				if(nextN.isInteger)
					sum += entry.getValue();
				else
					next.merge(nextN,entry.getValue(),Long::sum);
			}
			if(now.isInteger)
				sum++;
			else
				next.merge(now,1L,Long::sum);
			next.put(zero,sum);
			map = next;
			ans += sum;
		}
