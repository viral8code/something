	...
	public static void main ( String[] args ) {
		...
		while(T-->0){
			...
			if(N>5){
				...
				if(N<5){
					Loop:for(int i=1;i<1<<(1<<N);i+=2){
						if(Integer.bitCount(i)!=M)
							continue;
						int[] ans = new int[M];
						boolean[] set = new boolean[1<<(1<<N)];
						int index = 0;
						for(int j=0;j<1<<N;j++)
							if((i&(1<<j))>0)
								ans[index++] = j;
						...
						for(int j=0;j<M;j++)
							for(int k=0;k<M;k++)
								if(!set[ans[j]&ans[k]]||!set[ans[j]|ans[k]])
									continue Loop;
						out.println(ans);
					}
				}
					...
					switch(M){
					case 2:
						out.println(new int[]{0,31});
						break;
					case 3:
						out.println(new int[]{0,1,31});
						break;
					case 4:
						out.println(new int[]{0,1,30,31});
						break;
					case 5:
						out.println(new int[]{0,31});
						break;
