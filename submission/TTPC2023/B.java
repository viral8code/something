...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		for(int i=0;i<N;i++){
			...
			list.get((int)MathFunction.sum(subS[i])).add(i);
		}
		...
		while(deq.size()>0){
			...
			for(int i=(int)MathFunction.sum(subS[now])+2;
				i>=Math.max(0,(int)MathFunction.sum(subS[0])-2);i--){
				HashSet<Integer> subSet = new HashSet<>(list.get(i));
				for(int index:subSet){
					if(check(subS[now],subS[index])){
