...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		for(index=0;index<M;index++){
			if(!uf.unite(path[index][0],path[index][1])){
				flag = true;
				break;
			}
		}
		if(!flag)
			...
			for(int i=0;i<=index;i++){
				list.get(path[i][0]).add(new int[]{path[i][1],path[i][2]});
				list.get(path[i][1]).add(new int[]{path[i][0],path[i][2]});
			}
			...
			DFS:for(int[] p:path){
				...
				deq.add(p[0]);
				map.put(p[0],0);
				Loop:while(deq.size()>0){
					int now = deq.peekLast();
					if(flag2){
						cycle.add(deq.pollLast());
						if(now==num)
							break DFS;
						continue Loop;
					}
					for(int[] next:list.get(now)){
						if(map.containsKey(next[0])){
							if(Math.abs(map.get(next[0])-map.get(now))==next[1])
								continue;
							flag2 = true;
							num = next[0];
							cycle.add(deq.pollLast());
							...
							map.put(next[0],map.get(now)+next[1]);
							deq.add(next[0]);
							continue Loop;
						}
					}
			...
			for(int i=0;i<N;i++){
				if(cycle.contains(path[i][0])&&cycle.contains(path[i][1]))
					sum += path[i][2];
				else if(cycle.contains(path[i][0])||cycle.contains(path[i][1]))
					min = Math.min(path[i][2],min);
			}
			out.println(min==Integer.MAX_VALUE?-1:sum+min);
		}
