...
class Main{
	...
	public static void main(String[] args){
		...
		*/////////////
		...
		ArrayList<TreeSet<Integer>> boxes = new ArrayList<>(N+1);
		boxes.add(new TreeSet<Integer>());
		int[] list = new int[N+Q];
		for(int i=1;i<=N;i++){
		...
		int now = N+1;
		while(Q-->0){
			int op = System.in.nextInt();
			if(op==1){
				int X = System.in.nextInt();
				int Y = System.in.nextInt();
				TreeSet<Integer> newSet = boxes.get(X);
				TreeSet<Integer> oldSet = boxes.get(Y);
				for(int num:oldSet)
					list[num] = X;
				if(newSet.size()<oldSet.size()){
					TreeSet<Integer> temp = newSet;
					newSet = oldSet;
					oldSet = temp;
				}
				newSet.addAll(oldSet);
				boxes.set(X,newSet);
				boxes.set(Y,new TreeSet<>());
			}else if(op==2){
				int X = System.in.nextInt();
				list[now] = X;
				boxes.get(X).add(now);
