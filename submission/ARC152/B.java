...
class Main{
	...
	public static void main(String[] args){
		...
		*/////////////
		...
		long dist1 = 2*a[0];
		long dist2 = 2*(L-a[N-1]);
		for(int i=0;i<N;i++){
			long temp = L*2+Math.max(0,dist1-(a[i]-a[0]))+Math.max(0,dist2-dist1)+Math.max(0,a[i]-a[0]-dist2);
			if(ans>temp)
				ans = temp;
			temp = L*2+Math.max(0,dist2-(a[N-1]-a[i]))+Math.max(0,dist1-dist2)+Math.max(0,a[N-1]-a[i]-dist1);
			if(ans>temp)
				ans = temp;
			if(System.numSearch(a,L-a[i]))
				ans = L*2;
		}
...
class Library{
	...
	public static boolean numSearch(int[] nums,int num){
		int a=0,b=nums.length-1,ans=0,c;
		while(a-b<1){
			c=(a+b)/2;
			if(nums[c]>=num)b=c-1;
			else a=(ans=c)+1;
