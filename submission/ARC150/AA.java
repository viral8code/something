	...
	public static void main(String[] args)throws IOException{
		...
		*/////////////
		...
		while(T-->0){
			...
			int M = 0;
			for(int i=0;i<N;i++)if(S[i]=='1')M++;
			for(int i=1;i<=N;i++)sum0[i] += sum0[i-1] + (S[i-1]=='0'?1:0);
			for(int i=1;i<=N;i++)sum1[i] += sum1[i-1] + (S[i-1]=='1'?1:0);
			int count = 0;
			for(int i=K;i<=N;i++)
				if(sum0[i]-sum0[i-K]==0&&sum1[i]-sum1[i-K]==M)
					count++;
...
class UnionFind{
	...
	private ArrayList<HashSet<Integer>> routeList;
	...
	public UnionFind(int N){
		...
		rank = new int[N];
		size = new int[N];
		routeList = new ArrayList<HashSet<Integer>>();
		for(int i=0;i<N;i++)routeList.add(new HashSet<Integer>());
	...
	public boolean unite(int x,int y){
		...
		if(rank[rx]==rank[ry])
			rank[rx]++;
		size[rx] += size[ry];
		routeList.get(x).add(y);
		routeList.get(y).add(x);
		groupList.get(rx).addAll(groupList.get(ry));
		groupList.set(ry,null);
		--count;
		return true;
	}
	public ArrayList<HashSet<Integer>> getAllRoute(){
		return routeList;
	}
	public HashSet<Integer> getRoute(int x){
		return routeList.get(x);
	}
