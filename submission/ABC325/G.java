		...
		Loop:while(flag){
			...
			for(int i=1;i<S.length();i++){
				if(S.charAt(i-1)=='o'&&S.charAt(i)=='f'){
					int index = S.indexOf("of",i);
					if(MathFunction.rangeCheck(index,i,i+K)){
						S = S.substring(0,i-1)+S.substring(index);
						flag = true;
						out.println("of!");
						continue Loop;
					}
					if(i>1&&S.charAt(i-2)=='o'){
						for(int j=Math.min(S.length()-i-1,K+1);j>0;j--){
							if(S.charAt(i+j)=='f'){
					...
					out.println("K!");
					S = S.substring(0,i-1)+S.substring(Math.min(S.length(),i+K+1));
					flag = true;
					continue Loop;
				}
			}
