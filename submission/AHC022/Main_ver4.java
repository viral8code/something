...
final class Main {
	...
	/**
	 * tester.rsのメモ
	 * 68行目「input.parse::<TesterInput>()?」入力ファイルの内容から。
	 * 出口の座標(outCoordinate)、出口と対応する番号(outIdx)、ノイズ(noises)を生成してる。
	 * どうやらジャッジ側で標準偏差から計算しているわけではなく、すでにシードから計算済みらしい。
	 * 出典はlib.rsの335行目「checked_add(self.noises[self.measure_cnt - 1])」より。ただの足し算だね。
	 * 実際こんなことあるのか知らないけど、一応上下限が決められてるっぽい。assertみたいなもんか？
	 * 測定コスト(measureCost)は100*(10+abs(y)+abs(x))と書かれてて、
	 * 問題文とも一致してるのでこの解釈であってそう。
	 * 配置コスト(arrangeCost)も同様で、lib.rsの289行目のcalc_arrange_cost関数に定義されている。
	 * じゃあ、ランダムチェックに関してはin.txtの中身でこねこねするだけで良さそう。
	 */

	public static void main ( String[] args ) {

		SimpleScanner sc = new SimpleScanner( System.in );
		SimplePrinter out = new SimplePrinter( System.out, true );

		/**
		 * L*Lの二次元グリッド上の温度を適当にセットして、それを元に結果を推定して返すらしい。
		 * これ、Nが100しかないから計測回数を無視していいなら各10回ずつ計測して真値推定すんのが筋か？
		 * 統計の講義でやったような気はするけど、n=10で有意な推定が得られるんかね？
		 * それとも、数カ所絞って測定回数を増やすのが良いのか？
		 * やたら分母デカいから、コストを抑えれば推定ミスってようが点数高そう。
		 * とりあえず、うろ覚えの知識で推定するか…。
		 * けどめんどくさいから最初はただ平均とって推定で。
		 * 
		 * 0、1000判定ならSに関わらずほぼいけそう。
		 * じゃあ、1点だけ1000にして数回測定する方法が使える？
		 * 正規分布に従うなら、真値±標準偏差*3に99%が収まるから、
		 * クソデカ温度はS*6にしておけば判別できるでしょ。
		 * 
		 * 枝切りするだけでクッソ伸びる。まぁ本質はここよね。
		 * Sがデカいともうどうしようもないね。0と1000の区別も付かないし…。
		 * 正規分布に従うなら、0は平均S*0.4くらいになりそう。ホンマか？
		 * Randomで試した感じはそう。けど、1～3個くらいのデータからどうこうするのは無理がある。
		 * 
		 * あぁ、1000のところ以外のところも温度調節して細かいところ得点稼ぐか。
		 */
		...
		Point mid = getPointOfMiddle(p,L);
		int temperature = Math.min(1000,S*7);
		int[][] P = S<12*12?getPForLittleCase(p,mid,temperature,L):getP(p,mid,temperature,L);
		out.println(P);
		int[][] ans = new int[N][N];
		if(S>16*16){
			...
			for(int i=0;i<N;i++){
				...
				for(int j=0;j<N;j++){
					if(set.contains(j))
						continue;
					for(int k=0;k<num/(N-set.size())/(N-i);k++){
						sum--;
						out.println(i+" "+(mid.x-p[j].x)+" "+(mid.y-p[j].y));
						int m = sc.nextInt();
						ans[i][j] += m;
					}
				}
	...
	private static Point getPointOfMiddle(Point[] p,int L){
		Point ans = new Point(0,0);
		long distSum = Long.MAX_VALUE;
		for(int i=0;i<L;i++){
			for(int j=0;j<L;j++){
				long sum = 0;
				for(Point pp:p)
					sum += Math.abs(pp.x-i)+Math.abs(pp.y-j);
				if(sum<distSum){
					distSum = sum;
					ans.x = i;
					ans.y = j;
	...
	private static int[][] getP(Point[] p,Point mid,int T,int L){
		int[][] P = new int[L][L];
		for(int i=0;i<L;i++)
			for(int j=0;j<L;j++)
				P[i][j] = mid.x==i&&mid.y==j?T:0;
		int num = T/7;
	...
	private static int[][] getPForLittleCase(Point[] p,Point mid,int T,int L){
		int[][] P = new int[L][L];
		for(int i=0;i<L;i++)
			for(int j=0;j<L;j++)
				P[i][j] = mid.x==i&&mid.y==j?T:0;
