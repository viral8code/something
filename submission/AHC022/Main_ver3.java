...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		int temperature = Math.min(1000,S*6);
		for(int i=0;i<L;i++){
			System.out.print(mid.x==i&&mid.y==0?temperature:0);
			for(int j=1;j<L;j++){
				System.out.print(" ");
				System.out.print(mid.x==i&&mid.y==j?temperature:0);
			}
			System.out.println();
		}
		int[][] ans = new int[N][N];
		if(S>529){
			int request = 10000/N/50;
			int len = 50;
			while(len<N&&request==10000/N/(len+1))
				len++;
			...
			for(int i=0;i<N;i++){
				for(int j=0;j<len;j++){
					...
					for(int k=0;k<request;k++){
						System.out.println(i+" "+(mid.x-p[j].x)+" "+(mid.y-p[j].y));
						ans[i][j] += sc.nextInt();
					}
				}
				int max = 0;
	...
	private static Point getPointOfMiddle(Point[] p,int L){
		...
		for(int i=0;i<L;i++){
			for(int j=0;j<L;j++){
				long sum = 0;
				for(Point pp:p)
					sum += (pp.x-i)*(pp.x-i)+(pp.y-j)*(pp.y-j);
				if(sum<distSum){
					distSum = sum;
					ans.x = i;
					ans.y = j;
