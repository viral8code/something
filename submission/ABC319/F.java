...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		Search:do{
			PriorityQueue<Integer> portion = new PriorityQueue<>((a,b)->Integer.compare(map[s[a]],map[s[b]]));
			PriorityQueue<Integer> enemy = new PriorityQueue<>((a,b)->Integer.compare(s[a],s[b]));
			boolean[] checked = new boolean[N+1];
			checked[1] = true;
			long now = 1;
			for(int next:list.get(1)){
				checked[next] = true;
				if(t[next]==1)
					enemy.add(next);
				else
			...
			while(enemy.size()+portion.size()>0){
				if(enemy.size()>0&&s[enemy.peek()]<=now){
					int num = enemy.poll();
					now += g[num];
				...
				else if(portion.size()>0){
					int num = portion.poll();
					now = Math.min(1_000_000_000,now*g[num]);
					for(int next:list.get(num)){
						if(checked[next])
							continue;
						checked[next] = true;
...
final class SimplePrinter extends PrintWriter {
	public SimplePrinter ( final PrintStream os ) {
		super( os );
	}

	public SimplePrinter ( final PrintStream os, final boolean bool ) {
		super( os, bool );
	}
