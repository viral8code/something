...
final class Main{
	...
	public static void main(String[] args){
		...
		int N = sc.nextInt();
		if(N<4){
			int len = (int)MathFunction.fact(N);
			int[][] A = sc.nextInt(len,len);
			int[] subMap = new int[N];
			Arrays.setAll(subMap,i->i);
			int[][] set = new int[len][];
			for(int i=0;i<len;i++){
				set[i] = subMap.clone();
				ArrayFunction.nextPermutation(subMap);
			}
			int[] map = new int[len];
			...
			Loop:do{
				for(int i=0;i<len;i++)
					for(int j=0;j<len;j++)
						for(int k=0;k<N;k++)
							if(set[map[i]][set[map[j]][k]]!=set[map[A[i][j]-1]][k])
								continue Loop;
				list.add(map.clone());
				for(int i=0;i<len;i++)
					list.get(list.size()-1)[i]++;
			}while(ArrayFunction.nextPermutation(map));
			Collections.sort(list,(a,b)->Arrays.compare(a,b));
