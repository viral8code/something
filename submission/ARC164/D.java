	...
	public static void main ( String[] args ) {
		...
		for(int i=0;i<N<<1;i++){
			if(T.charAt(i)=='?'){
				max++;
				min++;
				for(int j=Math.max(minus-N,-min);j<=Math.min(N-plus,max);j++){
					if(N+j!=0)
						dp1[i+1][N+j] += dp1[i][N+j-1];
					if(N+j<N<<1)
						dp1[i+1][N+j] += dp1[i][N+j+1];
					dp1[i+1][N+j] %= mod;
				}
		...
		long sum = 2;
		for(int i=1;i<N<<1;i++){
			for(int j=Math.max(i-N,-i);j<=Math.min(N-i,i);j++){
				sum += (long)Math.abs(j)*dp1[i+1][N+j];
				sum += (long)Math.abs(j)*dp2[i-1][N+j];
				sum %= mod;
			}
		}
		out.println(sum);
