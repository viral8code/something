...
class Main{
	...
	public static void main(String[] args)throws IOException{
		...
		while(N>1){
			answer++;
			Map.Entry<Integer,Integer> max = A.pollLastEntry();
			Map.Entry<Integer,Integer> min = A.firstEntry();
			int temp = max.getKey()%min.getKey();
			if(temp==0){
				N--;
				if(max.getValue()==1)continue;
				else{
					A.put(max.getKey(),max.getValue()-1);
					continue;
				}
			}
			if(A.get(temp)==null){
