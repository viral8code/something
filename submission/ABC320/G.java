...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		for(int i=0;i<=10;i++){
			...
			set.add(new BitSet(N));
			int now = -1;
			Loop:while(true){
				for(int j=0;j<list.size();j++){
					now = now/M*M+list.get(j).get(0);
					HashSet<BitSet> next = new HashSet<>();
					for(BitSet bit:set){
						if(list.get(j).size()==2){
							if(!bit.get(list.get(j).get(1))){
								bit.set(list.get(j).get(1));
								next.add(bit);
						...
						for(int k=1;k<list.get(j).size();k++){
							...
							if(bit.get(num))
								continue;
							BitSet nextBit = (BitSet)bit.clone();
							nextBit.set(num);
							if(nextBit.cardinality()==N)
								break Loop;
							next.add(nextBit);
						}
					}
