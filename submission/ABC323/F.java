	...
	public static void main ( String[] args ) {
		...
		if(XB!=XC){
			if(YB!=YC){
				...
				if(YA==YB&&((XB<XC&&XB<XA)||(XC<XB&&XA<XB)))
					ans1 += 2;
				ans1 += Math.abs(XB-XC);
				ans1 += 2;
				ans1 += Math.abs(YB-YC);
				long ans2 = Math.abs(XA-XB)+(YB!=YC?Math.abs(YA-YB-Long.signum(YB-YC)):1);
				if(XA==XB&&((YB<YC&&YB<YA)||(YC<YB&&YA<YB)))
					ans2 += 2;
				ans2 += Math.abs(YB-YC);
				ans2 += 2;
				ans2 += Math.abs(XB-XC);
			...
			long ans = Math.abs(XA-XB)+(YB!=YC?Math.abs(YA-YB-Long.signum(YB-YC)):1);
			if(XA==XB&&((YB<YC&&YB<YA)||(YC<YB&&YA<YB)))
				ans += 2;
			ans += Math.abs(YB-YC);
			out.println(ans);
