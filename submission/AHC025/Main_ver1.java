...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		int len = list.size();
		while(Q<compareCount(len)){
			len--;
		}
		ArrayList<Goods> temp = list;
		list = new ArrayList<>();
		for(int i=0;i<len;i++){
			list.add(temp.get(i));
		}
		for(int i=len;i<temp.size();i++){
			list.get(i%len).add(temp.get(i));
		}
		Collections.sort(list);
		int[] num = getRandom(list.size());
		long average = MathFunction.sum(num)/list.size();
		int[] ans = new int[N];
		int[] sum = new int[D];
		PriorityQueue<Integer> queue = new PriorityQueue<>((a,b)->Integer.compare(sum[a],sum[b]));
		for(int i=0;i<D;i++)
			queue.add(i);
