...
final class Main {
	...
	private static class Goods implements Comparable<Goods>{
		...
		public int compareTo(Goods g){
			if(--Q<0)
				return new Random(System.nanoTime()).nextInt(3)-1;
			out.print(size()+" "+g.size());
			out.print(" ");
			out.println(this+" "+g);
			char c = sc.nextChar();
			return c=='<'?-1:c=='='?0:1;
	...
	public static void main ( String[] args ) {
		...
		while(Q>0){
			int index = rm.nextInt(Math.max(1,rm.nextInt(D-1)));
			Goods small = list.remove(index);
			Goods great = list.remove(D-2);
			index = rm.nextInt(great.size());
			Integer num = great.remove(index);
			small.add(num);
			if(great.size()==0){
				index = rm.nextInt(small.size()-1);
				num = small.remove(index);
				great.add(num);
			}
			index = Collections.binarySearch(list,small);
			if(index<0)
				index = -index-1;
			list.add(index,small);
			index = Collections.binarySearch(list,great);
			if(index<0)
				index = -index-1;
			list.add(index,great);
		}
		int[] ans = new int[N];
		for(int i=0;i<D;i++){
			for(int index:list.get(i).list){
				ans[index] = i;
			}
		}
		out.println(ans);
	...
	private static int compareCount(int N){
		int[] array = new int[N];
	...
	private static int[] getRandom(int N){
		int[] ans = new int[N];
		for(int i=0;i<N;i++){
			long num = (long)(-100000*Math.log(1-rm.nextDouble()));
			while(100*N/D<num)
				num = (long)(-100000*Math.log(1-rm.nextDouble()));
			ans[i] = Math.max(1,(int)num);
		}
		Arrays.sort(ans);
		return ans;
	}
	private static int merge(int[] array){
		...
		int ans = merge(array1)+merge(array2);
		int i=0,j=0,k=0;
	...
	private static void reverseMerge(int[] array){
		if(array.length<2)
			return;
		int[] array1 = new int[array.length>>1];
		int[] array2 = new int[array.length-array1.length];
		...
		for(int i=0;i<Math.min(array1.length,array2.length);i++){
			array[i<<1] = array1[i];
			array[i<<1|1] = array2[i];
		}
		int index = Math.min(array1.length,array2.length)<<1;
...
/*////////////////////////////////////////////////
	* My Library *
	...
	@author viral
*/////////////////////////////////////////////////
final class BeamSearchUtility<E>{
	private final PriorityQueue<E> queue;
	private final Comparator<E> comp;
	public BeamSearchUtility(Comparator<E> c){
		comp = c;
		queue = new PriorityQueue<>(c.reversed());
	}
	public int compare(final E a,final E b){
		return comp.compare(a,b);
	}
	public ArrayList<E> get(ArrayList<E> list,int N){
		queue.addAll(list);
		ArrayList<E> ans = new ArrayList<>(N);
		while(N-->0)
			ans.add(queue.poll());
		return ans;
	}
...
final class Timer{
	...
	public Timer(final long s){
		beforeTime = startTime = s;
		modCount = 0;
	}
	public double lapTimeNano(){
		++modCount;
		final long now = System.nanoTime();
		final double ans = (now-beforeTime)/1e1;
		beforeTime = now;
		return ans;
	}
	public double lapTimeMicro(){
		++modCount;
		final long now = System.nanoTime();
		final double ans = (now-beforeTime)/1e3;
		beforeTime = now;
		return ans;
	}
	public double lapTimeMilli(){
		++modCount;
		final long now = System.nanoTime();
		final double ans = (now-beforeTime)/1e6;
		beforeTime = now;
		return ans;
	}
	public double lapTimeSecond(){
		++modCount;
		final long now = System.nanoTime();
		final double ans = (now-beforeTime)/1e9;
		beforeTime = now;
		return ans;
	}
	public double allTimeNano(final boolean update){
		final long now = System.nanoTime();
		if(update){
			++modCount;
			beforeTime = now;
		}
		return (now-startTime)/1e1;
	}
	public double allTimeMicro(final boolean update){
		final long now = System.nanoTime();
		if(update){
			++modCount;
			beforeTime = now;
	...
	public double allTimeMilli(final boolean update){
		final long now = System.nanoTime();
		if(update){
			++modCount;
			beforeTime = now;
	...
	public double allTimeSecond(final boolean update){
		final long now = System.nanoTime();
		if(update){
			++modCount;
			beforeTime = now;
	...
	public double averageTimeNano(){
		return (System.nanoTime()-startTime)/1e1/(++modCount);
	}
	public double averageTimeMicro(){
		return (System.nanoTime()-startTime)/1e3/(++modCount);
	}
	public double averageTimeMilli(){
		return (System.nanoTime()-startTime)/1e6/(++modCount);
	}
	public double averageTimeSecond(){
		return (System.nanoTime()-startTime)/1e9/(++modCount);
	}
}
