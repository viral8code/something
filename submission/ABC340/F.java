...
final class Main{
	...
	public static void main(String[] args){
		...
		long X = sc.nextLong();
		long Y = sc.nextLong();
		long gcd = MathFunction.gcd(Math.abs(X),Math.abs(Y));
		if(gcd>2){
		...
		try{
			...
			try{
				while(true){
					if(MathFunction.rangeCheckClose(g[1],-limit,limit)&&MathFunction.rangeCheckClose(g[2],-limit,limit)){
						System.out.println(g[2]+" "+g[1]);
						return;
					}
					long lcm = Math.multiplyExact(X/gcd,Y);
					g[1] = Math.subtractExact(g[1],lcm/X);
					g[2] = Math.subtractExact(g[2],lcm/Y);
				}
			}catch(Exception e){}
			while(true){
				if(MathFunction.rangeCheckClose(g[1],-limit,limit)&&MathFunction.rangeCheckClose(g[2],-limit,limit)){
				...
				long lcm = Math.multiplyExact(X/gcd,Y);
				g[1] = Math.addExact(g[1],lcm/X);
				g[2] = Math.addExact(g[2],lcm/Y);
			}
		}catch(Exception e){}
		out.println(-1);
	...
	private static long[] extGCD(long a, long b) {
		if (b == 0) {
			return new long[]{a,1,2};
		}
		long[] g = extGCD(b, a%b);
		long y = g[1];
		y -= Math.multiplyExact(a/b,g[2]);
		return new long[]{g[0],g[2],y};
	}
}
