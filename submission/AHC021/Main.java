...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		int[][] heap = new int[N][];
		for(int i=0;i<N;i++)
			heap[i] = sc.nextInt(i+1);

		PriorityQueue<int[]> queue = new PriorityQueue<>(
			(a,b)->{
				int num1 = Integer.compare(a[0],b[0])<<2;
				int num2 = Integer.compare(a[1],b[1])<<1;
				int num3 = Integer.compare(a[2],b[2]);
				return num1+num2+num3;
			}
		);
		for(int i=0;i<N;i++)
			for(int j=0;j<=i;j++)
				queue.add(new int[]{heap[i][j],i,j});
		int[][] priority = new int[N*(N+1)/2][2];
		...
		ArrayList<int[]> ans = makeAnswerByGreedy(N,heap,priority);
		ArrayList<int[]> subAns = makeAnswerBySelection(N,heap,priority);
		if(ans.size()>subAns.size())
			ans = subAns;
		subAns = makeAnswerBySortAndGreedy(N,heap);
		if(ans.size()>subAns.size())
			ans = subAns;
		subAns = makeAnswerBySortReverseAndGreedy(N,heap);
		if(ans.size()>subAns.size())
			ans = subAns;

		while((System.nanoTime()-sTime)/1e6<1900){
			ArrayList<int[]> randomAnswer = makeAnswerByRandomSelection(N,heap,priority);
			if(ans.size()>randomAnswer.size())
				ans = randomAnswer;
		}
	...
	private static ArrayList<int[]> makeAnswerBySortAndGreedy(int N,int[][] heap){
		int[][] newHeap = new int[N][];
		for(int i=0;i<N;i++)
			newHeap[i] = Arrays.copyOf(heap[i],i+1);
		ArrayList<int[]> ans = new ArrayList<>();
		for(int i=0;i<N;i++){
			for(int j=0;i+j<N;j++){
				for(int k=1;i+j+k<N;k++){
					if(newHeap[i+k-1][k-1]>newHeap[i+k][k]){
						int num = newHeap[i+k][k];
						newHeap[i+k][k] = newHeap[i+k-1][k-1];
						newHeap[i+k-1][k-1] = num;
						ans.add(new int[]{i+k,k,i+k-1,k-1});
					}
				}
			}
		}
		for(int i=0;i<N;i++){
			for(int j=i;j<N;j++){
				for(int k=1;j+k<N;k++){
					if(newHeap[i+k-1][i]>newHeap[i+k][i]){
						int num = newHeap[i+k][i];
						newHeap[i+k][i] = newHeap[i+k-1][i];
						newHeap[i+k-1][i] = num;
						ans.add(new int[]{i+k,i,i+k-1,i});
					}
				}
			}
		}
		while(fix(N,ans,newHeap));
		return ans;
	}
	private static ArrayList<int[]> makeAnswerBySortReverseAndGreedy(int N,int[][] heap){
		int[][] newHeap = new int[N][];
		for(int i=0;i<N;i++)
			newHeap[i] = Arrays.copyOf(heap[i],i+1);
		ArrayList<int[]> ans = new ArrayList<>();
	...
	private static ArrayList<int[]> makeAnswerBySelection(int N,int[][] heap,int[][] priority){
		int[][] newHeap = new int[N][];
		for(int i=0;i<N;i++)
			newHeap[i] = Arrays.copyOf(heap[i],i+1);
		ArrayList<int[]> ans = greedy(N,newHeap,priority);
		while(fix(N,ans,newHeap));
		return ans;
	}
	private static ArrayList<int[]> selection(int N,int[][] heap,int[][] priority){
		ArrayList<int[]> ans = new ArrayList<>();
		int X = 0;
		int Y = 0;
		for(int[] sel:priority){
			int x = sel[0];
			int y = sel[1];
			while(x>X){
				if(y==0){
					if(heap[x-1][y]>heap[x][y]){
						int num = heap[x][y];
						heap[x][y] = heap[x-1][y];
						heap[x-1][y] = num;
						ans.add(new int[]{x,y,x-1,y});
						x--;
					}
					else
						break;
					...
					if(heap[x-1][y-1]>heap[x-1][y]){
						int num = heap[x][y];
						heap[x][y] = heap[x-1][y-1];
						heap[x-1][y-1] = num;
						ans.add(new int[]{x,y,x-1,y-1});
						x--;
						y--;
					}
					else{
	...
	private static ArrayList<int[]> makeAnswerByRandomSelection(int N,int[][] heap,int[][] priority){
		int[][] newHeap = new int[N][];
		for(int i=0;i<N;i++)
			newHeap[i] = Arrays.copyOf(heap[i],i+1);
		int[][] newPriority = new int[priority.length][2];
		for(int i=0;i<priority.length;i++){
			newPriority[i][0] = priority[i][0];
			newPriority[i][1] = priority[i][1];
		}
		ArrayList<int[]> ans = random(N,newHeap,newPriority);
		while(fix(N,ans,newHeap));
		return ans;
	}
	private static ArrayList<int[]> random(int N,int[][] heap,int[][] priority){
		shuffle(priority);
		ArrayList<int[]> ans = new ArrayList<>();
		for(int[] sel:priority){
			int x = sel[0];
	...
	private static void shuffle(int[][] select){
		Random rm = new Random(System.nanoTime());
		int sum = rm.nextInt(10)+1;
		while(sum-->0){
			int index1 = rm.nextInt(select.length);
			int index2 = rm.nextInt(select.length);
			int[] temp = select[index1];
			select[index1] = select[index2];
			select[index2] = temp;
		}
	}
	private static ArrayList<int[]> makeAnswerByGreedy(int N,int[][] heap,int[][] priority){
		int[][] newHeap = new int[N][];
		for(int i=0;i<N;i++)
			newHeap[i] = Arrays.copyOf(heap[i],i+1);
		ArrayList<int[]> ans = greedy(N,newHeap,priority);
	...
	private static ArrayList<int[]> greedy(int N,int[][] heap,int[][] priority){
		ArrayList<int[]> ans = new ArrayList<>();
		for(int[] sel:priority){
			int x = sel[0];
			int y = sel[1];
	...
	private static boolean fix(int N,ArrayList<int[]> ans,int[][] heap){
		boolean flag = false;
		for(int i=0;i<N;i++){
			for(int j=0;j<=i;j++){
				int x = i;
