...
class Main{
	...
	final static Library System = new Library(java.lang.System.in,java.lang.System.out);

	final static Random dice = new Random();

	final static int odds = 10000;
	...
	public static void main(String[] args){
		...
		*/////////////
		...
		final class Deq{
			int[] list;
			int index;
			protected Deq(int[] nums){
				list = nums;
				index = 0;
			}
			final protected int poll(){
				return list[index++];
			}
			final protected int look(int n){
				return list[n+index];
			}
		...
		final Deq next = new Deq(System.in.nextInt(100));
		int[][] map = new int[10][10];
		while(next.size()>0){
			int position = System.in.nextInt();
			...
			int[][] ne = null;
			for(int i=0;i<4;i++){
				final int[][] temp = set(map,i);
				int ans = score(temp);
				if(score<ans||(score==ans&&dice.nextInt(2)==0)){
					score = ans;
					way = i;
					ne = temp;
				}else if(dice.nextInt(odds)==0){
					score = ans;
					way = i;
					ne = temp;
				}
			...
			switch(way){
				...
				case 3:
					...
					break;
				default:
					System.err.println("What's the fuck?!?!?!");
					System.exit(0);
			}
			System.out.flush();
		}
	...
	final private static int[][] set(int[][] map,int way){
		int[][] next = new int[10][10];
		if(way==0){
			final ArrayList<ArrayDeque<Integer>> stack = new ArrayList<ArrayDeque<Integer>>();
			for(int i=0;i<10;i++)stack.add(new ArrayDeque<Integer>());
			for(int i=0;i<10;i++)
				for(int j=0;j<10;j++)
					if(map[j][i]!=0)
						stack.get(i).add(map[j][i]);
			for(int i=0;i<10;i++){
				int index = 0;
				for(int n:stack.get(i))
					next[index++][i] = n;
			}
		}
		else if(way==1){
	...
	final private static int score(int[][] map){
		...
		for(int i=0;i<10;i++){
			for(int j=0;j<10;j++){
				if(checked[i][j]||map[i][j]==0)
					continue;
				checked[i][j] = true;
				int sum = dfs(i,j,map,checked);
				ans += sum*sum;
			}
		}
		return ans;
	}
	final private static int dfs(int x,int y,int[][] map,boolean[][] checked){
		int ans = 1;
		if(x>0&&map[x-1][y]==map[x][y]&&!checked[x-1][y]){
			checked[x-1][y] = true;
			ans += dfs(x-1,y,map,checked);
		}
		if(y>0&&map[x][y-1]==map[x][y]&&!checked[x][y-1]){
			checked[x][y-1] = true;
			ans += dfs(x,y-1,map,checked);
		}
		if(x<9&&map[x+1][y]==map[x][y]&&!checked[x+1][y]){
			checked[x+1][y] = true;
			ans += dfs(x+1,y,map,checked);
		}
		if(y<9&&map[x][y+1]==map[x][y]&&!checked[x][y+1]){
			checked[x][y+1] = true;
			ans += dfs(x,y+1,map,checked);
		}
		return ans;
