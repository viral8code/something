...
class subMain{
	...
	public boolean intervalCheck(int num,int s,int c,int ss,int cc){
		int max = Math.max(intervalS[s][ss],intervalC[c][cc]);
	...
	public int[] next(int s,int c){
		...
		if(s==-1&&c==-1){
			...
			for(int i=period+1;i<=period+40;i++){
				int ii = i % hook;
				if(now[ii][0]!=-1&&inventoryCheck(now[ii][0])){
					pickS = now[ii][0];
					flag = false;
					break;
				}
			...
			for(int i=0;i<intervalS.length;i++){
				for(int j=0;j<intervalC.length;j++){
					if(inventory[i][j]>0){
						int temp = cost[s][c][i][j];
						if(temp<small&&pickS!=i){
							small = temp;
							if(flag)
								pickS = i;
							pickC = j;
...
class Main{
	public static void main(String[] args)throws IOException{
		...
		while(AI.remain>0){
			...
			nowS = nextSC[0];
			nowC = nextSC[1];
			AI.process(nextSC[0],nextSC[1],AI.inventory[nextSC[0]][nextSC[1]]);
			AI.rejection(nextSC[0],nextSC[1]);
		}
