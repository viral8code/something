...
class Main{
	...
	public static void main(String[] args)throws IOException{
		...
		for(int i=0;i<2*N;i++){
			for(int j=i+1;j<2*N;j++){
				A[i][j] = System.in.nextInt();
			}
		}
		paired = new boolean[2*N];
		int ans = 0;
		paired[0] = true;
		for(int i=1;i<2*N;i++){
			paired[i] = true;
			int temp = process(2*N-2,A[0][i]);
	...
	public static int process(int nokori,int num){
		if(nokori==0)return num;
		int max = 0;
		for(int i=1;i<2*N;i++){
			...
			for(int j=i+1;j<2*N;j++){
				...
				paired[j] = true;
				max = Math.max(max,process(nokori-2,num^A[i][j]));
				paired[j] = false;
			}
			paired[i] = false;
			break;
