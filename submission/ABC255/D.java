...
class Main{
	...
	*/
	...
	public static int upsearch(int[] nums,int num){int a=0,b=nums.length-1;if(nums[a]>=num)return a;if(nums[b]<=num)return b;int ans=(a+b)/2;while(a<=b){if(nums[ans]==num)return ans;else if(nums[ans]<num)a=ans+1;else b=ans-1;ans=(a+b)/2;}if(nums[ans]<num)return ans+1;return ans;}
