...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		int W = sc.nextInt();
		int T = sc.nextInt();
		final char[][] map = sc.nextCharArray(H);
		int sX = -1;
		int sY = -1;
		int gX = -1;
		int gY = -1;
		HashMap<Integer,Point> point = new HashMap<>();
		int index = 0;
		for(int i=0;i<H;i++){
			for(int j=0;j<W;j++){
				if(map[i][j]=='S'){
				...
				if(map[i][j]=='o')
					point.put(index++,new Point(i,j));
			}
		}
		int[][][] dist = new int[index+1][][];
		final int[] dx = {1,-1,0,0};
		final int[] dy = {0,0,1,-1};
		point.put(index,new Point(sX,sY));
		for(Map.Entry<Integer,Point> entry:point.entrySet()){
			...
			while(deq.size()>0){
				...
				for(int i=0;i<4;i++){
					int nextX = x+dx[i];
					int nextY = y+dy[i];
					if(!check(nextX+1,H)||!check(nextY+1,W))
						continue;
					if(map[nextX][nextY]=='#')
						continue;
					if(cost[nextX][nextY]>cost[x][y]+1){
		...
		for(int i=1;i<1<<index;i++){
			for(int j=0;j<index;j++){
				...
				for(int k=0;k<index;k++){
					if((i&1<<k)>0)
						continue;
					if(dist[j][point.get(k).x][point.get(k).y]==Integer.MAX_VALUE)
						continue;
					int sum = dp[i][j]+dist[j][point.get(k).x][point.get(k).y];
					if(dp[i|1<<k][k]>sum)
						dp[i|1<<k][k] = sum;
				}
			}
		}
		int ans = dist[index][gX][gY]<=T?0:-1;
		for(int i=1;i<1<<index;i++)
			for(int j=0;j<index;j++)
				if(dp[i][j]!=Integer.MAX_VALUE&&dist[j][gX][gY]!=Integer.MAX_VALUE)
					if(dp[i][j]+dist[j][gX][gY]<=T)
