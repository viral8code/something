...
/           /      _,.‐'￣／⌒     ⌒＼
  ...
  /       ﾉ       ilﾞフ::⌒（__人__）⌒:: ＼
      ...
      /  iﾄヾヽ_/ｨ"＼         `ー'´      ／
*/
class AVLTree_Multi<E extends Comparable<? super E>>{
	private Node<E> root;
	...
	final public boolean add(final E x){
		boolean bool;
		if(root==null){
	...
	final public boolean remove(final E x){
		boolean bool;
	...
	final public boolean contains(final E x){
		if(root==null)return false;
		return root.find(x);
	}
	final public int count(final E x){
		if(root==null)return 0;
		return root.count(x);
	}
	final public E pollFirst(){
	...
	final public E ceiling(final E x){
		if(root==null)return null;
		return root.ceiling(x,null);
	}
	final public E floor(final E x){
		if(root==null)return null;
	...
	final private class Node<E extends Comparable<? super E>>{
		private E value;
		int height,count;
