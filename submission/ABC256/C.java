...
class Main{
	...
	public static void main(String[] args)throws IOException{
		...
		int ans = 0;
		for(int i1=1;i1<=Math.min(h[0],w[0]);i1++){
			for(int i2=1;i2<=Math.min(h[0]-i1,w[1]);i2++){
				for(int j1=1;j1<=Math.min(h[1],w[0]-i1);j1++){
					for(int j2=1;j2<=Math.min(h[1],w[1]-i2);j2++){
						int temp1 = h[0]-i1-i2 + h[1]-j1-j2;
						int temp2 = w[0]-i1-j1 + w[1]-i2-j2;
						if(w[2]-temp1==h[2]-temp2)ans++;
					}
				}
			}
		}
	...
	/*テンプレ一覧
		...
		・int downsearch(int[],num)		：二分探索でnumを探してindexを返す(見つからない場合はnum以下のインデックスを、num以下が無ければ-1)
		・int upsearch(int[],num)		：二分探索でnumを探してindexを返す(見つからない場合はnum以上のインデックスを、num以上が無ければ-1)
	*/
	...
	public static int downsearch(int[] nums,int num){int a=0,b=nums.length-1;if(nums[a]==num)return a;if(nums[a]>num)return -1;if(nums[b]==num)return b;int ans=(a+b)/2;while(a<=b){if(nums[ans]==num)return ans;else if(nums[ans]<num)a=ans+1;else b=ans-1;ans=(a+b)/2;}if(nums[ans]>num)return ans-1;return ans;}
	public static int upsearch(int[] nums,int num){int a=0,b=nums.length-1;if(nums[a]>=num)return a;if(nums[b]==num)return b;if(nums[b]<num)return -1;int ans=(a+b)/2;while(a<=b){if(nums[ans]==num)return ans;else if(nums[ans]<num)a=ans+1;else b=ans-1;ans=(a+b)/2;}if(nums[ans]<num)return ans+1;return ans;}
