		...
		*/////////////
		...
		while(T-->0){
			...
			if(K>=N){
				...
				if(K/(2*N)%2==1){
					K %= 2*N;
					SS = S+Sr.substring(Math.min((2*N-(int)K),N),N)+S.substring(Math.min(Math.max((N-(int)K),0),N))+Sr;
				}else{
					K %= 2*N;
					SS = Sr+S.substring(Math.min((2*N-(int)K),N),N)+Sr.substring(Math.min(Math.max((N-(int)K),0),N))+S;
				}
			}else{
				SS = Sr.substring(N-(int)K,N)+S;
			}
			if(check(SS))
