...
class Main{
	...
	public static void main(String[] args){
		...
		横一列は全部ひっくり返すわけで、それって隣同士が等しいかは変化しないってことよね。
		だからなんだって感じなんだけど、隣に同じのが存在しない時は上下だけ見れば良いよねって話。
		問題は、ひっくり返す回数なんだよな。反転して上下に等しい組が出来る、
		極端な話、(1,0,1),(1,0,1),(0,1,0),(0,1,0)とかで良いってワケ。
		じゃあどうするかなんだけど、隣同士が等しい列は除いて縦に11001100って並べられるかよね。
		いや、1111111とかでもいいか。じゃあどうすんだよ。
		制約から読み取るコーナー～～～
		O(HW)が間に合いそう！それしかわからん！さすがに二乗は無理。
		普通に貪欲やって、H/2よりデカければH-ansすれば良いのかな？(反転すれば良いだけなので)
		ん～･･･じゃあやってみるか。
		いや、愚直もダルいわ･･･なんだこれ･･･。
		んあ～根気に任せて実装すんぞ～～～！！！
		...
		*/////////////
		...
		int[][] map = System.in.nextInt(H,W);
		int ans = 0;
		boolean[] isGood = new boolean[W];
		for(int i=0;i<W;i++){
			int c1 = 0;
			int c2 = 0;
			for(int i=0;i<W;i++){
				if(j>0&&map[i][j-1]==map[i][j]||j<W-1&&map[i][j+1]==map[i][j])
					continue;
				if(i>0&&map
