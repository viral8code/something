...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		int N = sc.nextInt();
		int M = sc.nextInt();
		class Roulette implements Comparable<Roulette>{
			int cost;
			int[] number;
			public Roulette(int C,int[] S){
				cost = C;
				number = S;
			}
			public double expectedValue(int M){
				int sum = 0;
				for(int num:S)
					sum += Math.min(M,num);
				return (double)sum/number.length/cost;
			}
		}
		Roulette[] r = new Roulette[N];
		for(int i=0;i<N;i++){
			int C = sc.nextInt();
			int P = sc.nextInt();
			int[] S = sc.nextInt(P);
			MathFunction.sort(S);
			r[i] = new Roulette(C,S);
		}
		Roulette[] goodRoulette = new Roulette[M];
		for(int i=0;i<N;i++){
			final int num = M-i;
			Arrays.sort(r,(a,b)->{
				double numA = a.expectedValue(num);
				double numB = b.expectedValue(num);
				if(numA!=numB)
