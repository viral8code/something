...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		while(T-->0){
			...
			int[][] canMove = new int[N+1][2];
			ArrayDeque<Integer> deq = new ArrayDeque<>();
			deq.add(1);
			deq.add(0);
			while(deq.size()>0){
				int now = deq.pollFirst();
				int cost = deq.pollFirst();
				for(int next:list.get(now)){
					if(canMove[next][(cost+1)%2]==0){
						canMove[next][(cost+1)%2] = cost+1;
						deq.add(next);
						deq.add(cost+1);
					}
				}
			}
			out.println(canMove[1][0]%2==0||canMove[1][1]%5==0?"Yes":"No");
		}
