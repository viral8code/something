...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		while(M-->0){
			...
			int b = sc.nextInt();
			int c = sc.nextInt();
			list.get(u).add(new int[]{v,b,c});
		}
		long[][] max = new long[N+1][4];
		for(long[] temp:max)
			temp[1] = temp[3] = Long.MAX_VALUE>>1;
		max[1][1] = 0;
		for(int i=1;i<N;i++){
			for(int[] path:list.get(i)){
				double num1 = (double)max[path[0]][0]/max[path[0]][1];
				double num2 = (double)(max[i][0]+path[1])/(max[i][1]+path[2]);
				double num3 = (double)(max[i][2]+path[1])/(max[i][3]+path[2]);
				if(num1<Math.max(num2,num3)){
					max[path[0]][0] = (num2>num3?max[i][0]:max[i][2])+path[1];
					max[path[0]][1] = (num2>num3?max[i][1]:max[i][3])+path[2];
				}
				if(max[i][3]+path[2]<max[path[0]][3]){
					max[path[0]][2] = max[i][2]+path[1];
					max[path[0]][3] = max[i][3]+path[2];
				}
				else if(max[i][3]+path[2]==max[path[0]][3]){
					max[path[0]][2] = Math.max(max[i][2]+path[1],max[path[0]][2]);
				}
			}
		}
		out.println(Math.max((double)max[N][0]/max[N][1],(double)max[N][2]/max[N][3]));
