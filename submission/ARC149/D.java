...
class Main{
	...
	public static void main(String[] args)throws IOException{
		...
		*/////////////
		...
		int[] X = System.in.nextInt(N);
		int[] D = System.in.nextInt(M);
		...
		int[] answer = new int[(int)1e6+1];
		for(int i=0;i<M;i++){
			int d = D[i];
			HashMap<Integer,ArrayList<Integer>> next = new HashMap<Integer,ArrayList<Integer>>();
			for(int tmp:list.keySet()){
				ArrayList<Integer> point = list.get(tmp);
				if(Math.abs(tmp)==Math.abs(d)){
					for(int p:point)
						answer[p] = i+1;
				}
				else{
					if(tmp>0){
						if(next.get(tmp-d)!=null)
							next.get(tmp-d).addAll(point);
						else
							next.put(tmp-d,point);
					}
					else{
						if(next.get(tmp+d)!=null)
							next.get(tmp+d).addAll(point);
