	...
	public static void main(String[] args)throws IOException{
		...
		for(int i=0;i<T;i++){
			...
			if(isPrime(temp.length())){
				ans = 0;
				long num = Long.valueOf(temp.substring(0,1));
				...
				for(int j : list.keySet()){
					int tem = temp.length()/j;
					String str = "";
					for(int k=0;k<tem;k++){
						str += temp.substring(0,j);
					}
					seikei:for(int s=0;s<(temp.length()-j);s+=j){
						for(int k=(j-1+s);k>=s;k--){
							int a = Integer.parseInt(str.substring(k,k+1));
							int b = Integer.parseInt(temp.substring(k,k+1));
							if(N>=Long.valueOf(str))
								break seikei;
							else if(a>=b){
								int number = 0;
								if((k%j)==0){
									number = Integer.parseInt(str.substring((k%j),(k%j)+1))-1;
									if(number<=0){
										str = "0";
										break seikei;
									}
									...
									number = Integer.parseInt(str.substring(0,j))-(int)Math.pow(10,j-(k%j))/10;
									if(String.valueOf(number).length()<j){
										str = "0";
										break seikei;
									}
								}
								for(int l=(k%j);l<str.length();l+=j){
									str = str.substring(0,(l/j)*j)+number+str.substring(l+1,str.length());
								}
							}
						}
					}
				...
				for(int nKey : list.keySet()){
					if(ans<Long.valueOf(list.get(nKey))){
						ans = Long.valueOf(list.get(nKey));
					}
				}
			}
			println(ans);
