...
class Main{
	...
	static int X,M,C,E,V,D;
	static long score;
	static int[][] costA,costB,RO;
	static double[][] RD;
	...
	public static void main(String[] args)throws IOException{
		...
		for(int i=0;i<M;i++){
			for(int j=0;j<9;j++){
				costA[i][j] = System.in.nextInt();
				costB[i][j] = System.in.nextInt();
			}
		}
		RD = new double[M][X];
		RO = new int[M][X];
		int[] answer = new int[2*X+1];
		for(int i=1;i<=2*X;i++)answer[i] = 9;
		int point = 1;
		while(E-->0){
			for(int i=0;i<M;i++){
				for(int j=2*X;j>0;j--)System.out.print(answer[j]);
				System.out.println();
			}
			System.out.flush();
			score = System.in.nextLong();
			V = System.in.nextInt();
			D = System.in.nextInt();
			for(int i=0;i<M;i++){
				for(int j=0;j<X;j++){
					RD[i][j] = Double.parseDouble(System.in.next());
					RO[i][j] = System.in.nextInt();
				}
			}
			//TODO
			//ここでなんか上手いことやる
			if(trigger){
				if(time<4.0){
					...
					if(score>0){
						while(answer[2*X-(7-point)]==1&&point>0)point-=2;
						if(point<=0){
							trigger = false;
						}else{
							for(int i=2*X-(7-point);i>0;i-=7){
								answer[i]--;
								answer[i-1]--;
							}
						}
