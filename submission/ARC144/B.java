...
class Main{
	public static void main(String[] args)throws IOException{
		...
		SimpleScanner ss = new SimpleScanner();
		int N = ss.nextInt();
		int a = ss.nextInt();
		int b = ss.nextInt();
		int[] A = new int[N];
		for(int i=0;i<A.length;i++)A[i] = ss.nextInt();
		Sys.sort(A);
		int answer = A[0];
		while(true){
			int temp = Math.max((A[A.length-1]-A[0])/(a+b),1);
			A[A.length-1]-=temp*b;
			A[0]+= temp*a;
			if(A[A.length-2]>A[A.length-1]||A[0]>A[1]){
				if(Math.max(A[0],A[1])>=Math.max(A[A.length-2],A[A.length-1]))
					break;
				Sys.sort(A);
			}
			if(answer<A[0])
...
*/////////////////////////////////////////////////
class SimpleScanner{
	private final int buff_size=(int)Library.pow(2,20);
	private final InputStream is=System.in;
	private byte[] buff=new byte[buff_size+2];
	private int length=0,point=0;
	public SimpleScanner()throws IOException{reload();}
	...
	private void load(int n)throws IOException{
		...
		for(int i=point,j=0;i<length+2;i++,j++)buff[j]=buff[i];
		length-=point;
		point=0;
	...
	public String next()throws IOException{
		...
		while(buff[point]>32)sb.append((char)buff[point++]);
		return sb.toString();
	}
	public String toString(){
		StringBuilder answer = new StringBuilder();
