...
class Main{
	...
	public static void main(String[] args){
		...
		*/////////////
		...
		A = System.in.nextInt(N);
		int answer = dfs(X,0,0);
		System.out.println(answer);

		System.out.close();
	}
	public static int dfs(int money,int now,int buy){
		if(now==A.length)
			return buy;
		String s = String.valueOf(money);
		int[] nums = new int[s.length()];
		for(int i=0;i<nums.length;i++)nums[i] = -(s.charAt(i)-'0');
		System.sort(nums);
		ArrayList<ArrayList<Integer>> list = permutation(nums, new ArrayList<ArrayList<Integer>>());
		int ans = dfs(money,now+1,buy);
		int max = -1;
		for(ArrayList<Integer> num:list){
			...
			for(int n:num)
				mon = mon*10-n;
			if(mon>=A[now]){
				char[] temp = String.valueOf(mon-A[now]).toCharArray();
				int k = 0;
				Arrays.sort(temp);
		...
		return max!=-1?dfs(max,now+1,buy+1):ans;
	}
	private static ArrayList<ArrayList<Integer>> permutation(int[] seed,ArrayList<ArrayList<Integer>> list) {
		int[] perm = new int[seed.length];
		boolean[] used = new boolean[seed.length];
		buildPerm(seed, perm, used, 0, list);
		return list;
	}
	private static void buildPerm(int[] seed, int[] perm, boolean[] used, int index,ArrayList<ArrayList<Integer>> list) {
		...
		for (int i = 0; i < seed.length; i++) {
			...
			perm[index] = seed[i];
			used[i] = true;
			buildPerm(seed, perm, used, index + 1,list);
			used[i] = false;
		}
	}
}
