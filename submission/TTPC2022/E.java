...
class Main{
	...
	public static void main(String[] args)throws IOException{
		...
		*/////////////
		...
		char[] subA = br.readLine().toCharArray();
		char[] subB = new StringBuilder(br.readLine()).reverse().toString().toCharArray();
		HashMap<Character,ArrayList<Integer>> A = new HashMap<>();
		for(int i=0;i<subA.length;i++){
			ArrayList<Integer> temp = A.get(subA[i]);
			if(temp==null){
		...
		HashMap<Character,ArrayList<Integer>> B = new HashMap<>();
		for(int i=0;i<subB.length;i++){
			ArrayList<Integer> temp = B.get(subB[i]);
			if(temp==null){
				temp = new ArrayList<Integer>();
				B.put(subB[i],temp);
			}
		...
		int Q = System.parseInt(br.readLine());
		while(Q-->0){
			...
			for(int i=0;i<N&&index1<subA.length;i++){
				ArrayList<Integer> temp = A.get(S.charAt(index1));
				Integer count = list.getOrDefault(S.charAt(index1),0);
				if(temp!=null&&count<temp.size()&&m<temp.get(count)){
					m = temp.get(count);
					count++;
					list.put(S.charAt(index1),count);
					index1++;
				}else
					break;
			}
			...
			int index2 = N;
			m = subB.length;
			for(int i=0;i<N&&index2>0;i++){
				ArrayList<Integer> temp = B.get(S.charAt(index2-1));
				Integer count = list.getOrDefault(S.charAt(index2-1),0);
				if(temp!=null&&count<temp.size()&&m>temp.get(temp.size()-count-1)){
					m = temp.get(temp.size()-count-1);
					count++;
					list.put(S.charAt(index2-1),count);
					index2--;
				}else
					break;
			}
			index2 = N-index2;
			if(index1+index2>=N&&index1>0&&index2>0){
				System.out.println(Math.max(N%2,N-2*Math.min(index1,index2)));
			}else{
...
class SimpleScanner{
	...
	public String nextLine(boolean bool){
		StringBuilder ans = new StringBuilder();
		byte c = read();
		while(bool&&c<=' ')c = read();
		while(c!='\n'){
