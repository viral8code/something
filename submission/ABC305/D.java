...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		map.put(0,0);
		for(int i=1;i<N;i+=2)
			map.put(A[i],map.lastEntry().getValue()+(A[i+1]-A[i]));
		int Q = sc.nextInt();
		while(Q-->0){
			int l = sc.nextInt();
			int r = sc.nextInt();
			Integer indexL = map.ceilingKey(l);
			if(indexL==null){
				out.println(Math.max(0,Math.min(r,A[N-1])-l));
				continue;
			}
			int indexR = map.floorKey(r);
			int subL = Searcher.upSearch(A,indexL);
			int subR = Searcher.upSearch(A,indexR);
			int L = map.get(indexL);
			int R = map.get(indexR);
			if(indexL>0){
				L -= A[subL+1]-A[subL];
				if(A[subL-1]>l)
					L -= A[subL-1]-l;
			}
			if(subR<N-1&&A[subR+1]>r)
				R -= A[subR+1]-r;
			out.println(Math.max(0,R-L));
		}
