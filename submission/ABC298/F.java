...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		HashMap<Integer,Long> mapC = new HashMap<>();
		while(N-->0){
			int r = sc.nextInt();
			int c = sc.nextInt();
			long x = sc.nextInt();
			map.put(new Point(r,c),x);
			mapR.merge(r,x,(i,j)->i+j);
			mapC.merge(c,x,(i,j)->i+j);
		}
		RCSet[] setR = new RCSet[mapR.size()];
		int index = 0;
		for(Map.Entry<Integer,Long> e:mapR.entrySet())
			setR[index++] = new RCSet(e.getKey(),0,e.getValue());
		RCSet[] setC = new RCSet[mapC.size()];
		index = 0;
		for(Map.Entry<Integer,Long> e:mapC.entrySet())
			setC[index++] = new RCSet(0,e.getKey(),e.getValue());
		Arrays.sort(setR,Comparator.reverseOrder());
		Arrays.sort(setC,Comparator.reverseOrder());
		PriorityQueue<RCSet> queue = new PriorityQueue<>();
		for(RCSet R:setR){
			for(RCSet C:setC){
				long value = R.value+C.value;
				if(queue.size()==N){
					if(queue.peek().value<value){
						queue.poll();
						queue.add(new RCSet(R.r,C.c,value));
					}
					else
						break;
				}
		...
		long ans = 0;
		for(RCSet rc:queue)
			ans = Math.max(ans,rc.value-map.getOrDefault(new Point(rc.r,rc.c),0L));
		out.println(ans);

		out.close();
	}
}
class RCSet implements Comparable<RCSet> {
	int r,c;
	long value;
	public RCSet(int R,int C,long v){
		r = R;
		c = C;
		value = v;
	}
	@Override
	public int compareTo(RCSet rc){
		long num = Long.signum(value-rc.value)<<2;
		num += Long.signum(r-rc.r)<<1;
		num += Long.signum(c-rc.c);
