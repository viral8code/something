...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		Point[][] p = new Point[301][4];
		HashSet<Integer> set = new HashSet<>();
		for(int i=0;i<N;i++){
			for(int j=0;j<M;j++){
				set.add(A[i][j]);
				if(p[A[i][j]][0]==null){
					p[A[i][j]][0] = new Point(i+1,j+1);
				}
				else if(p[A[i][j]][0].x<i+1){
					p[A[i][j]][0] = new Point(i+1,j+1);
				}
				if(p[A[i][j]][1]==null){
					p[A[i][j]][1] = new Point(i+1,j+1);
				}
				else if(p[A[i][j]][1].x>i+1){
					p[A[i][j]][1] = new Point(i+1,j+1);
				}
				if(p[A[i][j]][2]==null){
					p[A[i][j]][2] = new Point(i+1,j+1);
				}
				else if(p[A[i][j]][2].y<j+1){
					p[A[i][j]][2] = new Point(i+1,j+1);
				}
				if(p[A[i][j]][3]==null){
					p[A[i][j]][3] = new Point(i+1,j+1);
				}
				else if(p[A[i][j]][3].y>j+1){
					p[A[i][j]][3] = new Point(i+1,j+1);
				}
			}
		}
		...
		long ans = 0;
		for(int num:set){
			int max = sum[N][M]+sum[p[num][0].x-1][p[num][0].y-1]-sum[p[num][0].x-1][p[num][0].y]-sum[p[num][0].x][p[num][0].y-1];
			max = Math.max(max,sum[p[num][0].x-1][p[num][0].y-1]);
