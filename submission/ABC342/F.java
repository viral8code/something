...
final class Main{
	...
	public static void main(String[] args){
		...
		int L = sc.nextInt();
		int D = sc.nextInt();
		LazySegmentTree<Double,Double> lSegT = new LazySegmentTree<>(N+D,0.0,0.0,false){
			public Double function(Double a,Double b){
				return a+b;
			}
			public Double mapping(Double a,Double f){
				return a+f;
			}
		...
		lSegT.update(0,1.0);
		for(int i=0;i<N;i++){
			lSegT.apply(i+1,i+D+1,lSegT.get(i)/D);
		}
		double ans = lSegT.query(1,N+1)*lSegT.query(N+1,N+D);
		for(int i=L+1;i<=N;i++)
			ans += lSegT.get(i)*lSegT.query(L,i);
		out.println(ans);
