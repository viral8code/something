...
final class Main{
	...
	private static class F{
		final long p,q,sub,invQ;
		F(long p,long q,long sub){
			this.p = p;
			this.q = q;
			this.sub = sub;
			invQ = MathFunction.modPow(q,MOD-2,MOD);
		}
	}
	...
	public static void main(String[] args){
		...
		LazySegmentTree<Long,F> lSegT = new LazySegmentTree<>(A,0L,origin,false){
			...
			public F composition(F a,F b){
				long q = a.q*b.q%MOD;
				long p = a.p*b.sub%MOD+a.q*b.p%MOD;
				if(MOD<=p)
					p -= MOD;
				long sub = a.sub*b.sub%MOD;
				return new F(p,q,sub);
			}
		};
		while(M-->0){
			int L = sc.nextInt();
			int R = sc.nextInt();
			int X = sc.nextInt();
			lSegT.apply(L-1,R,new F(X,R-L+1,R-L));
		}
		long[] ans = new long[N];
		for(int i=0;i<N;i++)
			ans[i] = lSegT.get(i);
