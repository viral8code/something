...
final class Main{
	...
	public static void main(String[] args){
		...
		System.arraycopy(sum,0,dp,0,len);
		System.arraycopy(sum,0,next,0,len);
		for(int i=2;i<=D;i++){
			for(int j=1;j<len;j++){
				int S = (j-1)&j;
				while(S>0){
					long num = dp[j^S]+sum[S];
					if(num<next[j])
						next[j] = num;
					S = (S-1)&j;
				}
		...
		long allSum = MathFunction.sum(W);
		out.println((dp[len-1]*D-allSum*allSum)/(double)D/D);
