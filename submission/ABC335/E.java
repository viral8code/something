...
final class Main{
	...
	public static void main(String[] args){
		...
		for(int[] p:path){
			int U = p[0];
			int V = p[1];
			int C = Math.min(scc.id(U),scc.id(V));
			int D = Math.max(scc.id(U),scc.id(V));
			if(C!=D)
				list.get(C).add(D);
		}
		int[] ans = new int[N+1];
		ans[scc.id(1)] = 1;
		for(int i=scc.id(1);i<scc.id(N);i++)
			for(int next:list.get(i))
				ans[next] = Math.max(ans[next],ans[i]+1);
		out.println(ans[scc.id(N)]);
