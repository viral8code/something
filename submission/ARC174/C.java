...
final class Main{
	...
	private static final Factorial fact = new Factorial(2_000_000,MOD);
	...
	public static void main(String[] args){
		...
		long turnB = 0;
		for(int i=0;i<N;i++){
			long temp1 = (long)N*i%MOD*i%MOD;
			temp1 *= MathFunction.modPow(fact.getInverse(N+i),2,MOD)*fact.getInverse(N-i)%MOD;
			temp1 %= MOD;
			temp1 += temp1*N%MOD*fact.getInverse(i)%MOD;
			temp1 %= MOD;
			long temp2 = (long)i*i%MOD*i%MOD;
			temp2 *= MathFunction.modPow(fact.getInverse(N+i),2,MOD)*fact.getInverse(N-i)%MOD;
			temp2 %= MOD;
			temp2 += temp2*N%MOD*fact.getInverse(i)%MOD;
			temp2 %= MOD;
			A += turnA*temp1%MOD;
			A += turnB*temp2%MOD;
			A %= MOD;
			...
			B += turnA*temp2%MOD;
			B %= MOD;
			long nextTurnA = turnB*N%MOD*fact.getInverse(N+i)%MOD + turnA*N%MOD*fact.getInverse(N+i)%MOD*i%MOD*fact.getInverse(N)%MOD;
			long nextTurnB = turnA*N%MOD*fact.getInverse(N+i)%MOD + turnB*N%MOD*fact.getInverse(N+i)%MOD*i%MOD*fact.getInverse(N)%MOD;
			turnA = nextTurnA;
			turnB = nextTurnB;
		}
		out.println(A+" "+B);
