...
class Main{
	...
	public static void main(String[] args)throws IOException{
		...
		while((java.lang.System.nanoTime()-startTime)/1e9<4.5){
			for(int i=0;i<=N/2;i++){
				for(int j=i;j<N-i;j++){
					if(isMarked[i][j])
						continue;
	...
	private static boolean isOverlapped(int x1,int y1,int x2,int y2,int x3,int y3,int x4,int y4){
		...
		if(type1==3){
			if(x1==x3)
				result = Math.signum(x1-x2)==Math.signum(x1-x4);
			else if(x2==x3)
				result = Math.signum(x2-x1)==Math.signum(x2-x4);
			else if(x1==x4)
				result = Math.signum(x1-x2)==Math.signum(x1-x3);
			else if(x2==x4)
				result = Math.signum(x2-x1)==Math.signum(x2-x3);
			else{
				int subX = x1-x2<0 ? 1 : -1;
				int subY = y1-y2<0 ? 1 : -1;
				int x = x1;
				int y = y1;
				while(x!=x2){
					if(x==x4&&y==y4){
		...
		return type1==type2&&
			(type1==1 ? x1==x3&&!(Math.max(y1,y2)<Math.min(y3,y4)||Math.max(y3,y4)<Math.min(y1,y2)) :
			 ...
			 (y2-y1)/(x2-x1)==(y4-y3)/(x4-x3)&&result);
	}
