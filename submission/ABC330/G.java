...
final class Main{
	...
	public static void main(String[] args){
		...
		Factorial fact = new Factorial(3000,MOD);
		int N = sc.nextInt();
		int[] A = sc.nextInt();
		...
		for(int num:A){
			if(num>0){
				ans += bit.sum(N)-bit.sum(num);
				ans %= MOD;
				bit.add(num,1);
			}
			else{
				for(int sub:list){
					ans += bit.sum(N)-bit.sum(sub);
					ans %= MOD;
				}
				subSum *= N;
				subSum %= MOD;
				for(int i=list.size()-1;i>0;i++){
					subSum += fact.getCombi(list.size(),count);
...
/**
 * int特化のHashSetクラスです。
 * java.util.HashSet<Integer>よりも高速な処理を提供します。
 */
final class IntegerSet{
	/**
	 * このSetの各値を保持するNodeクラスです。
	 */
	private class Node{
	...
	/**
	 * 引数の値を追加します。
	 * 引数の値が既に追加されている場合は変更されません。
	 *
	 * @param x 追加する値
	 *
	 * @return 引数の値が既に追加されている場合はfalse、それ以外はtrue
	 */
	...
	/**
	 * 引数の値を削除します。
	 * このIntegerSetに引数の値が無い場合は変更されません。
	 *
	 * @param x 削除する値
	 *
	 * @return 引数の値が存在していたならtrue、それ以外はfalse
	 */
	...
	/**
	 * このIntegerSetの要素数を返します。
	 *
	 * @return このIntegerSetの要素数
	 */
	public int size(){
		return size;
	}
	...
	/**
	 * このIntegerSetに存在する要素を配列にして返します。
	 * 順序は保証されません。
	 *
	 * @return このIntegerSetを表す配列
	 */
	public int[] toArray(){
	 ...
	 * このメソッドはjava.util.Arrays.toString(toArray())と等価です。
	 *
	 * @return このIntegerSetを表すString
	 */
