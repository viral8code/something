...
class Main{
	...
	public static void main(String[] args){
		...
		*/////////////
		...
		ArrayList<ArrayList<Integer>> route0 = new ArrayList<ArrayList<Integer>>();
		ArrayList<ArrayList<Integer>> route1 = new ArrayList<ArrayList<Integer>>();
		for(int i=0;i<=N;i++){
			route0.add(new ArrayList<Integer>());
			route1.add(new ArrayList<Integer>());
		}
		while(M-->0){
		...
		HashSet<Integer> swi = new HashSet<Integer>();
		while(K-->0){
		...
		int max = (int)1e9;
		Arrays.fill(cost0,max);
		Arrays.fill(cost1,max);
		cost1[1] = 0;
		while(bfs.size()>0){
			int now = bfs.pollFirst();
			int flag = bfs.pollFirst();
			if(flag==0){
				for(int next:route0.get(now)){
					if(cost0[next]>cost0[now]+1){
						cost0[next] = cost0[now]+1;
						bfs.add(next);
						bfs.add(flag);
					}
				}
				if(swi.contains(now)&&cost0[now]<cost1[now]){
					cost1[now] = cost0[now];
					bfs.add(now);
					bfs.add(1);
				}
			}else{
				for(int next:route1.get(now)){
					if(cost1[next]>cost1[now]+1){
						cost1[next] = cost1[now]+1;
						bfs.add(next);
						bfs.add(flag);
					}
				}
				if(swi.contains(now)&&cost1[now]<cost0[now]){
					cost0[now] = cost1[now];
					bfs.add(now);
					bfs.add(0);
				}
		...
		int ans = Math.min(cost0[N],cost1[N]);
