...
class Main{
	...
	public static void main(String[] args){
		...
		*/////////////
		...
		final int N = System.in.nextInt();
		int M = System.in.nextInt();
		int K = System.in.nextInt();
		final IntList[] route = new IntList[2*N+1];
		for(int i=0;i<=N*2;i++){
			route[i] = new IntList();
		}
		while(M-->0){
		...
		final IntQueue bfs = new IntQueue();
		bfs.add(N+1);
		final int[] cost = new int[2*N+1];
		final int max = (int)1e9;
		Arrays.fill(cost,max);
		cost[N+1] = 0;
		while(!bfs.empty()){
			final int now = bfs.poll();
			route[now].set();
			while(route[now].hasNext()){
				final int next = route[now].next();
		...
		final int ans = Math.min(cost[N],cost[2*N]);
...
final class IntQueue{
	...
	final public int addFirst(int x){
		head--;
		if(head<0)
			head += size;
		queue[head] = x;
	...
	final private void grow(){
		if(head>0){
			final int[] newQueue = new int[size];
