...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		int N = sc.nextInt();
		int M = sc.nextInt();
		long K = (long)N*M-sc.nextLong()+1;
		int[][] suger1 = sc.nextInt(N,2);
		int[][] suger2 = sc.nextInt(M,2);
		Arrays.sort(suger1,(s1,s2)->{
			int num = Integer.compare(s1[0],s2[0]);
			return num==0?Integer.compare(s1[1],s2[1]):num;
		});
		Arrays.sort(suger2,(s1,s2)->{
			int num = Integer.compare(s1[0],s2[0]);
			return num==0?-Integer.compare(s1[1],s2[1]):num;
		});
		int[] index = new int[N];
		Arrays.fill(index,M-1);
		Search:for(int i=N-1;i>=0;i--){
			for(int j=M-1;j>=0;j--){
				double num = func(suger1[i],suger2[j]);
				PriorityQueue<Double> queue = new PriorityQueue<>(Comparator.reverseOrder());
				for(int k=0;k<i;k++){
					double n;
					while(0<=index[k]&&(n=func(suger1[k],suger2[index[k]]))>=num){
						queue.add(n);
						index[k]--;
					}
				}
	...
	private static double func(int[] s1,int[] s2){
		return (double)(s1[0]+s2[0])/(s1[0]+s1[1]+s2[0]+s2[1]);
	}
}
