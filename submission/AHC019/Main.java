...
//////////////////考察メモ(ver2.0より)///////////////////////////////////////////////////////////////////////////////////////////
		/* 
		 * ○～ver2.0
		 * ブロックをクソデカにして提出。サンプルと同じ得点だった。
		 * とりあえず、全ブロック1*1*1で構築して提出。サンプルと同じ得点だった。
		 * ここで気付く、誤読の可能性。どうやら共通ブロックをかなり多く、
		 * そしてブロックをデカくするらしい。なんでちゃんと読まないんですか？
		 * というわけで、1*1*1のサイズでブロック作って、可能な限り共通にさせる。
		 * すると半分くらいの得点になった。
		 * で、今度はシルエット別にブロックを数えて、多い方を少ない方に合わせるように
		 * ブロックを削減させた。ちょっと減った。
		 * ここで、とりあえずブロックは減らした方がよくね？ってなって全力で消してみた。
		 * 3分の1くらいになった。わ～い。
		 * 次はブロックを適当に連結させてみた。横方向に伸ばせるだけ伸ばして共通ブロックに。
		 * さらに3分の1くらいになった。最高。今12位じゃん。今終わってくれ。
		 * 
		 * 
		 * ○ver2.0
		 * 
...
//////////////////入力パート/////////////////////////////////////////////////////////////////////////////////////////////////////
		int D = sc.nextInt();
		boolean[][] f1 = new boolean[D][D];
		for(int i=0;i<D;i++){
			for(int j=0;j<D;j++){
				f1[i][j] = sc.nextChar()=='1';
			}
		}
		boolean[][] r1 = new boolean[D][D];
		for(int i=0;i<D;i++){
			for(int j=0;j<D;j++){
				r1[i][j] = sc.nextChar()=='1';
			}
		}
		boolean[][] f2 = new boolean[D][D];
		for(int i=0;i<D;i++){
			for(int j=0;j<D;j++){
				f2[i][j] = sc.nextChar()=='1';
			}
		}
		boolean[][] r2 = new boolean[D][D];
		for(int i=0;i<D;i++){
			for(int j=0;j<D;j++){
				r2[i][j] = sc.nextChar()=='1';
			}
		}
...
//////////////////処理1//////////////////////////////////////////////////////////////////////////////////////////////////////////
		/*
		 * ブロックを置いてはいけない箇所は頂点0と連結させることで
		 * 無視させる
		 */
		UnionFind uf1 = new UnionFind(D*D*D+1);
		UnionFind uf2 = new UnionFind(D*D*D+1);
		for(int i=0;i<D;i++){
			for(int j=0;j<D;j++){
				for(int k=0;k<D;k++){
					if(!f1[i][j]||!r1[i][k])
						uf1.unite(0,i*D*D+j*D+k+1);
					if(!f2[i][j]||!r2[i][k])
						uf2.unite(0,i*D*D+j*D+k+1);
				}
			}
		}
		/* 変数
		 * 
		 * UnionFind uf1,uf2	:ブロックの連結を管理する用のUnionFind
		 * 
		 */
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				for(int k=0;k<D;k++){
					if(f1[i][j]&&r1[i][k]){
						...
						for(int jj=0;jj<D;jj++){
							if(j==jj)
								continue;
							flag |= uf1.root(i*D*D+jj*D+k+1)>0;
						}
						if(!flag)
							continue;
						flag = false;
						for(int kk=0;kk<D;kk++){
							if(k==kk)
								continue;
							flag |= uf1.root(i*D*D+j*D+kk+1)>0;
						}
						if(flag)
							uf1.unite(0,i*D*D+j*D+k+1);
					}
				...
				for(int k=0;k<D;k++){
					if(f2[i][j]&&r2[i][k]){
						...
						for(int jj=0;jj<D;jj++){
							if(j==jj)
								continue;
							flag |= uf2.root(i*D*D+jj*D+k+1)>0;
						}
						if(!flag)
							continue;
						flag = false;
						for(int kk=0;kk<D;kk++){
							if(k==kk)
								continue;
							flag |= uf2.root(i*D*D+j*D+kk+1)>0;
						}
						if(flag)
							uf2.unite(0,i*D*D+j*D+k+1);
					}
...
//////////////////処理3//////////////////////////////////////////////////////////////////////////////////////////////////////////
		/*
		 * 主に棒状のブロックばかりできるので
		 * 各棒状ブロックの長さと始点の座標を全列挙する
		 */
		class Point3D implements Comparable<Point3D>{
			int i,j,k,num;
			Point3D(int n,int x,int y,int z){
				num = n;
				i = x;
				j = y;
				k = z;
			...
			public int compareTo(Point3D p){
				int result = Integer.compare(num,p.num);
				if(result==0)
					result = Integer.compare(i,p.i);
				if(result==0)
					result = Integer.compare(j,p.j);
				if(result==0)
					result = Integer.compare(k,p.k);
				return result;
			}
		}
		PriorityQueue<Pair<Integer,Point3D>> map1 = new PriorityQueue<>(Comparator.reverseOrder());
		PriorityQueue<Pair<Integer,Point3D>> map2 = new PriorityQueue<>(Comparator.reverseOrder());


		//シルエット1
		for(int i=0;i<D;i++){
			boolean[][] checked = new boolean[D][D];
			for(int j=0;j<D;j++){
				for(int k=D-1;k>=0;k--){
					if(uf1.root(i*D*D+j*D+k+1)>0&&!checked[j][k]){
						checked[j][k] = true;
						int sum1 = 0;
						for(int jj=j;jj<D;jj++){
							if(uf1.root(i*D*D+jj*D+k+1)==0)
								break;
							checked[jj][k] = true;
							sum1++;
						}
						...
						for(int kk=k;kk>=0;kk--){
							if(uf1.root(i*D*D+j*D+kk+1)==0)
								break;
							checked[j][kk] = true;
							sum2++;
						}
						if(sum1<sum2)
							map1.add(new Pair<>(sum2,new Point3D(1,i,j,k)));
						else
							map1.add(new Pair<>(sum1,new Point3D(0,i,j,k)));
					}
				}
			}
		}
		...
		for(int i=0;i<D;i++){
			...
			for(int j=0;j<D;j++){
				for(int k=D-1;k>=0;k--){
					if(uf2.root(i*D*D+j*D+k+1)>0&&!checked[j][k]){
						checked[j][k] = true;
						int sum1 = 0;
						for(int jj=j;jj<D;jj++){
							if(uf2.root(i*D*D+jj*D+k+1)==0)
								break;
							checked[jj][k] = true;
							sum1++;
						}
						int sum2 = 0;
						for(int kk=k;kk>=0;kk--){
							if(uf2.root(i*D*D+j*D+kk+1)==0)
								break;
							checked[j][kk] = true;
							sum2++;
						}
						if(sum1<sum2)
							map2.add(new Pair<>(sum2,new Point3D(1,i,j,k)));
						else
							map2.add(new Pair<>(sum1,new Point3D(0,i,j,k)));
					}
				}
			}
		}
		/* クラス
		 * 
		 * Point3D implements Comparable<Point3D>
		 * 	num	:j方向なら0、k方向なら1
		 * 	i,j,k	:始点の座標
		 * 
		 */
		...
		/* 変数
		 * 
		 * PriorityQueue<Pair<Integer,Point3D>> map1,map2	:棒状ブロックの長さがkey、方向と始点がvalue
		 * 
		 */
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////処理4//////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		while(Math.min(map1.size(),map2.size())>0){
			Pair<Integer,Point3D> m1 = map1.poll();
			Pair<Integer,Point3D> m2 = map2.poll();
			int len = Math.min(m1.getKey(),m2.getKey());
			if(len<=1)
				break;
			int x1 = m1.getValue().i;
			int y1 = m1.getValue().j;
			int z1 = m1.getValue().k;
			int x2 = m2.getValue().i;
			int y2 = m2.getValue().j;
			int z2 = m2.getValue().k;
			while(--len>0){
				if(m1.getValue().num==0){
					uf1.unite(x1*D*D+y1*D+z1+1,x1*D*D+(y1+len)*D+z1+1);
				}
				else{
					uf1.unite(x1*D*D+y1*D+z1+1,x1*D*D+y1*D+z1+1-len);
				}
				if(m2.getValue().num==0){
					uf2.unite(x2*D*D+y2*D+z2+1,x2*D*D+(y2+len)*D+z2+1);
				}
				else{
					uf2.unite(x2*D*D+y2*D+z2+1,x2*D*D+y2*D+z2+1-len);
				}
			}
			set1.put(uf1.root(x1*D*D+y1*D+z1+1),index);
			set2.put(uf2.root(x2*D*D+y2*D+z2+1),index);
			index++;
		}
		/* 変数
		 * 
		 * TreeMap<Integer,Integer> set1,set2	:各シルエットでのUnionFindのrootとブロック番号の対応付け用
		 * int index				:ブロック番号用
		 * 
		 */
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////出力パート/////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		for(int i=0;i<D;i++){
			for(int j=0;j<D;j++){
				for(int k=0;k<D;k++){
					if(!set1.containsKey(uf1.root(i*D*D+j*D+k+1)))
						set1.put(uf1.root(i*D*D+j*D+k+1),index2++);
				}
			}
		}
		index2 = index;
		...
		for(int i=0;i<D;i++){
			for(int j=0;j<D;j++){
				for(int k=0;k<D;k++){
					if(!set2.containsKey(uf2.root(i*D*D+j*D+k+1)))
						set2.put(uf2.root(i*D*D+j*D+k+1),index2++);
				}
			}
		}
		 ...
		 * 答えを配列に格納
		 */
		int[] b1 = new int[D*D*D];
		for(int i=0;i<D;i++){
			for(int j=0;j<D;j++){
				for(int k=0;k<D;k++){
					b1[j*D*D+k*D+i] = set1.get(uf1.root(i*D*D+j*D+k+1));
				}
			}
		}
		int[] b2 = new int[D*D*D];
		for(int i=0;i<D;i++){
			for(int j=0;j<D;j++){
				for(int k=0;k<D;k++){
					b2[j*D*D+k*D+i] = set2.get(uf2.root(i*D*D+j*D+k+1));
				}
			}
		}
		 ...
		 * 答えの出力
		 */
		out.println(Math.max(set1.size(),set2.size())-1);
		out.println(b1," ");
		out.println(b2," ");
		out.close();
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
