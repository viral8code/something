...
final class ArrayTree<E>{
	...
	public void merge(final ArrayTree<E> x){
		if(root==null)
			root = x.root;
		else{
			Node par = getLastNode(root);
	...
	public boolean removeAll(final int l){
		Node subRoot = root;
		reverseCheck(root);
		int sumL = 0;
		while(sumL<l){
	...
	private void toList(Node now,java.util.ArrayList<E> list){
		if(now==null)
			return;
		reverseCheck(now);
		toList(now.left,list);
		list.add(now.value);
		toList(now.right,list);
	}
	...
	public boolean equals(final Object o){
		if(o instanceof final ArrayTree<?> tree){
			if(size!=tree.size()){
				return false;
			}
