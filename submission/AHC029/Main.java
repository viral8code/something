...
final class Solver{
	private int N,M,K,T;
	private long money;
	private PriorityQueue<Integer> t0,t1,t2,t3,t4;
	private PriorityQueue<Project> queue;
	private boolean triggerOfReverseOrder;

	private class Project implements Comparable<Project>{
		long h,v;
		public Project(long h,long v){
			this.h = h;
			this.v = v;
		}
		@Override
		public int compareTo(Project p){
			...
			double b = p.v/p.h;
			if(a==b)
				return Long.compare(p.h,h);
			return Double.compare(a,b);
		}
	}

	public Solver(){
		t0 = new PriorityQueue<>(Comparator.reverseOrder());
		t1 = new PriorityQueue<>(Comparator.reverseOrder());
		t2 = new PriorityQueue<>(Comparator.reverseOrder());
		t3 = new PriorityQueue<>(Comparator.reverseOrder());
		t4 = new PriorityQueue<>(Comparator.reverseOrder());
		queue = new PriorityQueue<>();
	}
	public void init(SimpleScanner sc){
		N = sc.nextInt();
		M = sc.nextInt();
		K = sc.nextInt();
		T = sc.nextInt();
		...
		for(int i=0;i<M;i++){
			long h = sc.nextLong();
			long v = sc.nextLong();
			queue.add(new Project(h,v));
		}
	}
	public void solve(SimpleScanner sc,SimpleWriter out){
	}
}
