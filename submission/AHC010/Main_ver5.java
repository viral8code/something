				...
				for(int j=1;j<tail[i].length-1;j++){
					if(tail[i][j]==0&&
						...
						answer = answer.substring(0,i*30+j)+
							String.valueOf((Integer.parseInt(answer.substring(i*30+j,i*30+j+1))+1)%4)+
					...
					else if((random.nextInt(100)+1)<=10){
						int temp = random.nextInt(4)%4;
						answer = answer.substring(0,i*30+j)+
							String.valueOf(temp)+
							answer.substring(i*30+j+1,answer.length());
						if(tail[i][j]<4)
							tail[i][j] = (tail[i][j] + 1)%4;
						else if(tail[i][j]==4||tail[i][j]==6)
							tail[i][j]++;
						else
							tail[i][j]--;
					}
