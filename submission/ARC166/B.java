...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		int N = sc.nextInt();
		int[] abc = sc.nextInt(3);
		long[] A = sc.nextLong(N);
		class Data{
			HashMap<Integer,Integer> map;
			long cost = 0;
			boolean[] checked = new boolean[3];
			public Data(HashMap<Integer,Integer> m){
				map = m;
			}
		}
		PriorityQueue<Data> queue = new PriorityQueue<>((a,b)->Long.compare(a.cost,b.cost));
		long[] costs = new long[1<<3];
		Arrays.fill(costs,Long.MAX_VALUE);
		queue.add(new Data(new HashMap<>()));
		while(costs[7]==Long.MAX_VALUE){
			Data d = queue.poll();
			int index = 0;
			for(int i=0;i<3;i++)
				if(d.checked[i])
					index += 1<<i;
			if(costs[index]==Long.MAX_VALUE){
				costs[index] = d.cost;
				for(int i=0;i<3;i++){
					if((index|(1<<i))==index)
						continue;
					for(int j=0;j<N;j++){
						long bef = 1;
						int befIndex = d.map.getOrDefault(j,0);
						for(int k=0;k<3;k++)
							if((befIndex&(1<<k))>0)
								bef = MathFunction.lcm(bef,abc[k]);
						Data next = new Data(new HashMap<>(d.map));
						next.map.put(j,befIndex|(1<<i));
						long num = (A[j]+bef-1)/bef*bef;
						bef = MathFunction.lcm(bef,abc[i]);
						long nextNum = (num+bef-1)/bef*bef;
						next.cost = costs[index]+nextNum-num;
						next.checked = d.checked.clone();
						next.checked[i] = true;
						queue.add(next);
					}
				}
