...
class Main{
	...
	public static void main(String[] args){
		...
		*/////////////
		...
		Test:while(T-->0){
			...
			ArrayDeque<ArrayList<Integer>> color = new ArrayDeque<>();
			ArrayDeque<HashSet<Integer>> visited = new ArrayDeque<>();
			bfs.add(1);
			ArrayList<Integer> temp = new ArrayList<>();
			temp.add(C[0]);
			color.add(temp);
			HashSet<Integer> temps = new HashSet<>(N);
			temps.add(1);
			visited.add(temps);
			int ans = Integer.MAX_VALUE;
			while(!bfs.isEmpty()){
				int p = bfs.pollFirst();
				ArrayList<Integer> list = color.pollFirst();
				HashSet<Integer> set = visited.pollFirst();
				if(set.size()>=ans)
					continue;
				if(p==N){
					if(check(list)){
						ans = Math.min(ans,set.size()-1);
					}
				}
				for(int next:route.get(p)){
					if(!set.contains(next)){
						HashSet<Integer> nextSet = new HashSet<>(set);
						nextSet.add(next);
						ArrayList<Integer> nextList = new ArrayList<>(list);
						nextList.add(C[next-1]);
						color.add(nextList);
						visited.add(nextSet);
						bfs.add(next);
					}
	...
	private static final boolean check(ArrayList<Integer> list){
		if(list.size()%2==1)
			return false;
		int N = list.size();
		for(int i=0;i<N;i++)
