...
final class Main {
	...
	private static Point[] broadcasters,houses;
	private static int[][] path;

	private static final Random rm = new Random();
	private static final int TIME_LIMIT = 1750;

	/* メモ
		solver0:->309165227
		全部5000

		solver1:->365754094->412022233(すまんバグってた())
		放送局の数がN、放送局間の辺の数がMで、それぞれon、offが切り替えられる。
		K箇所に住民がいて、全員に届けたい。
		ふむ、住民全員に対する一番近い放送局はO(NK)で求められるのか。
		NK<=5*10^5だから十分間に合うね。
		辺のon、offにコストはwかかるから、最小全域木の要領で組むのが最適なんだけど、
		使わなくて良い頂点を含めた方が良い場合もあるわけで、その判定が難しいな…。
		一度結ぶ毎に全部が連結になっているかを判定するのはO(N)かかって、全体でO(NM)。
		間に合うな？じゃあこれで最初行ってみようか。

		refinement:->424508944->424555380(出力の調整をちゃんとした)
		せっかく作ったsolver1だし、これをベースに改良を加えたいなぁ。
		最小全域木の辺で、明らかに無駄な辺は存在するんだよな。
		けど直すのめんどくさ…。
		じゃあ評価関数作ってこれ改良するか。

		再改良:
		多分最短頂点を選んでるから、もっと少ない頂点でやればもっと綺麗にはなりそうだよね～
		例えば、放送局と放送局の間よりもそれぞれに寄った点があるとき、それはどっちかで補った方が
		得点が高くなる場合があるわけで、それが考慮できたらな～とは思う。
	 */
	...
	public static void main ( String[] args ) {
		...
		int[] B = new int[M];
		solver1(P,B);
		while(refinement(P,B,timer));
		double score = score(P,B);
		int[] originalP = Arrays.copyOf(P,N);
		int[] originalB = Arrays.copyOf(B,M);
		while(!timer.timeOver(TIME_LIMIT)){
			if(rm.nextInt(3)==0)
				breaker(B);
			else
				overlap(B);
			fitting(P,B);
			while(refinement(P,B,timer));
			double nextScore = score(P,B);
			if(nextScore>score){
				originalP = Arrays.copyOf(P,N);
				originalB = Arrays.copyOf(B,M);
			}
			else{
				P = Arrays.copyOf(originalP,N);
				B = Arrays.copyOf(originalB,M);
			}
		}
		out.println(originalP);
		out.println(originalB);

		out.close();
	}

	private static void overlap(int[] B){
		int[] deq = new int[N];
		int index = 0;
		for(int i=0;i<M;i++)
			if(B[i]==1)
		...
		for(int i=0;i<index;i++){
			for(int j=i+1;j<index;j++){
				long dist = dist(
					broadcasters[i].x-broadcasters[j].x,
					broadcasters[i].y-broadcasters[j].y
				);
				if(dist<min){
					min = dist;
					num = rm.nextInt(2)==0?i:j;
				}
				else if(rm.nextInt(15)==0){
					min = dist;
					num = rm.nextInt(2)==0?i:j;
				}
			}
	...
	private static void fitting(int[] P,int[] B){
		int[] count = new int[N];
		for(int i=0;i<M;i++){
			if(B[i]==1){
				count[path[i][0]]++;
		...
		for(int i=0;i<M;i++){
			if(B[i]==1){
				list[path[i][0]][--count[path[i][0]]] = path[i][1];
				list[path[i][1]][--count[path[i][1]]] = path[i][0];
			}
		}
		...
		for(Point p:houses){
			...
			for(int i=0;i<tail;i++){
				long dist = dist(broadcasters[deq[i]].x-p.x,broadcasters[deq[i]].y-p.y);
				if(min>dist){
					min = dist;
					index = deq[i];
				}
				else if(rm.nextInt(5)==0){
					min = dist;
					index = deq[i];
				}
			}
			P[index] = Math.max(P[index],getPower(min));
		}
	}

	private static void breaker(int[] B){
		int[] deq = new int[N];
		int index = 0;
		for(int i=0;i<M;i++)
			if(B[i]==1)
				deq[index++] = i;
		B[index=rm.nextInt(index)] = 0;
	}

	private static boolean refinement(int[] P,int[] B,Timer timer){
		boolean flag = false;
		for(int i=0;i<M;i++){
			if(B[i]==1){
				double score = score(P,B);
				...
				for(int j=0;j<M;j++)
					if(B[j]==1)
						uf.unite(path[j][0],path[j][1]);
				int p1 = P[path[i][0]];
				int p2 = P[path[i][1]];
				if(!uf.isSame(0,path[i][0]))
					P[path[i][0]] = 0;
				if(!uf.isSame(0,path[i][1]))
					P[path[i][1]] = 0;
				double nextScore = score(P,B);
				if(score>nextScore){
					P[path[i][0]] = p1;
	...
	private static double score(int[] P,int[] B){
		long ans = 0;
		for(int num:P)
			ans += num*num;
		for(int i=0;i<M;i++)
		...
		for(int i=0;i<K;i++){
			...
			for(int j=0;j<N;j++){
				if(canGo[j])
					canListen |= dist(houses[i].x-broadcasters[j].x,houses[i].y-broadcasters[j].y)<=P[j]*P[j];
			}
			if(canListen)
				n++;
		}
		...
		if(n<K)
			...
			answer = 1e6*(1+1e8/(ans+1e7));
			
		return answer;
	}

	private static void solver1(int[] P,int[] B){
		boolean[] set = new boolean[N];
		for(Point p:houses){
			int index = -1;
			long min = Long.MAX_VALUE;
			for(int i=0;i<N;i++){
				long dist = dist(broadcasters[i].x-p.x,broadcasters[i].y-p.y);
				if(min>dist){
					min = dist;
					index = i;
				}
		...
		int[][] queue = new int[M][];
		for(int i=0;i<M;i++)
			queue[i] = new int[]{path[i][0],path[i][1],path[i][2],i};
		Arrays.sort(queue,(a,b)->Integer.compare(a[2],b[2]));
		int index = 0;
		UnionFind uf = new UnionFind(N);
		while(!check1(uf,set)){
			int[] path = queue[index++];
			if(uf.unite(path[0],path[1]))
				B[path[3]] = 1;
		}
	}

	private static int getPower(long dist){
		int num = (int)Math.sqrt(dist);
		if(num*num<dist)
			num++;
		if((num-1)*(num-1)>=dist)
			num--;
		return Math.min(5000,num);
	}
	...
	private static boolean check1(UnionFind uf,boolean[] set){
		int root = -1;
		for(int i=0;i<N;i++)
			if(set[i]&&root!=uf.root(i)){
				if(root==-1)
					root = uf.root(i);
				else
					return false;
	...
	private static void input(){
		...
		M = sc.nextInt();
		K = sc.nextInt();
		broadcasters = sc.nextPoint(N);
		path = sc.nextInt(M,3);
		for(int i=0;i<M;i++)
			for(int j=0;j<2;j++)
				path[i][j]--;
		houses = sc.nextPoint(K);
	}
}
class Timer{
	long num;
	...
	public boolean timeOver(int t){
		return (System.nanoTime()-num)/1e6>=t;
	}
}
...
final class Searcher {
	 ...
	 * @return arrayにvalueが含まれているかを表すboolean
	 */
	public static boolean search ( long[] array, long value ) {
		int a = 0, b = array.length - 1, c;
		while ( a - b < 1 ) {
			c = ( a + b ) / 2;
			if ( array[c] > value ) {
	 ...
	 * @return arrayにvalueが含まれているかを表すboolean
	 */
	public static <E extends Comparable<E>> boolean search ( E[] array, E value ) {
		int a = 0, b = array.length - 1, c;
		while ( a - b < 1 ) {
			c = ( a + b ) / 2;
	 ...
	 */
	public static <E extends Comparable<E>> boolean search ( List<E> list, E value ) {
	 ...
	 */
	public static boolean search ( int a, int b, int value, IntUnaryOperator func ) {
		int c;
		while ( a - b < 1 ) {
			c = ( a + b ) / 2;
	 ...
	 * @return この関数がa以上b以下で探索値を取るかを表すboolean
	 */
	public static boolean search ( long a, long b, long value, LongUnaryOperator func ) {
		long c;
		while ( a - b < 1 ) {
			c = ( a + b ) / 2;
