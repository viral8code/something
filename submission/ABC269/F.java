...
class Main{
	...
	public static void main(String[] args)throws IOException{
		...
		*/////////////
		...
		oddSum[0] = oddSum[1] = (long)(N+1)/2*((N+1)/2+1)/2*M%mod;
		evenSum[1] = evenSum[2] = (long)(N+2)/2*((N+2)/2+1)/2*M%mod;
		for(int i=2;i<N;i+=2){
			oddSum[i] = (oddSum[i-1] + M*2)%mod;
			if(i+1<N){
				oddSum[i+1] = oddSum[i];
				evenSum[i+1] = (evenSum[i] + M*2)%mod;
				if(i+2<N)
		...
		while(Q-->0){
			...
			long answer = oddSum[B-1]+evenSum[B-1];
			answer -= A>1?oddSum[A-2]+oddSum[A-2]:0;
			answer -= C>1?(C-1)*((long)(A-1)*M*2+C+(C%2)+1-(A+C%2)):0;
			answer -= D<M?(M-D)*((long)(A-1)*M*2+D+(D%2)+1-(A+D%2)):0;
			answer %= mod;
