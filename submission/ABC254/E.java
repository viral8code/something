...
class Main{
	...
	public static void main(String[] args)throws IOException{
		...
		for(int i=0;i<M;i++){
			String[] temp = reads();
			ab[0][i] = Integer.parseInt(temp[0]);
			ab[1][i] = Integer.parseInt(temp[1]);
		}
		int Q = readInt();
		for(int i=0;i<Q;i++){
			String[] temp = reads();
			...
			dfs(Integer.parseInt(temp[1]),Integer.parseInt(temp[0]),list);
			int ans = 0;
	...
	public static void dfs(int count,int number,ArrayList<Integer> list){
		if(count==0) return;
		count--;
		for(int i=0;i<ab[0].length;i++){
			if(ab[0][i]==number){
				if(!list.contains(ab[1][i])){
					list.add(ab[1][i]);
					dfs(count,ab[1][i],list);
				}
			}
			else if(ab[1][i]==number){
				if(!list.contains(ab[0][i])){
					list.add(ab[0][i]);
					dfs(count,ab[0][i],list);
				}
			}
		}
		return;
