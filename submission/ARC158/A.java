...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		while(T-->0){
			...
			int x2 = sc.nextInt();
			int x3 = sc.nextInt();
			long min = Math.min(x1,Math.min(x2,x3));
			long max = Math.max(x1,Math.max(x2,x3));
			long mid = (long)x1+x2+x3-min-max;
			long sum = 2*max-mid-min;
			if(sum%6==0&&(max-mid)%2==0&&(mid-min)%2==0){
				if(max-mid>=(mid-min)<<1)
					out.println(sum/6);
				else{
					long ans = (max-mid)/2;
					mid += 2*ans;
					min += 4*ans;
					ans += Math.abs(mid-min)/3;
					out.println(ans);
				}
			}else
				out.println(-1);
