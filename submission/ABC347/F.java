...
final class Main{
	...
	public static void main(String[] args){
		...
		long[][] list = new long[(N-M+1)*(N-M+1)][];
		int index = 0;
		for(int i=M;i<=N;i++)
			for(int j=M;j<=N;j++)
				list[index++] = new long[]{sum[i][j]-sum[i-M][j]-sum[i][j-M]+sum[i-M][j-M],i,j};
		Arrays.sort(list,(a,b)->Long.compare(b[0],a[0]));
		PriorityQueue<long[]> queue2 = new PriorityQueue<>((a,b)->Long.compare(b[0],a[0]));
		PriorityQueue<long[]> queue3 = new PriorityQueue<>((a,b)->Long.compare(b[0],a[0]));
		long ans = 0;
		for(long[] now:list){
			if((System.nanoTime()-s)/1e9>2800)
				break;
			if(queue3.size()>0&&queue3.peek()[0]+now[0]<=ans)
				break;
			PriorityQueue<long[]> next2 = new PriorityQueue<>((a,b)->Long.compare(a[0],b[0]));
			PriorityQueue<long[]> next3 = new PriorityQueue<>((a,b)->Long.compare(a[0],b[0]));
			while(queue2!=null&&queue2.size()>0){
				long[] sub = queue2.poll();
				if(dist(sub[1],sub[2],now[1],now[2])>=M)
					next3.add(new long[]{now[0]+sub[0],now[1],now[2],sub[1],sub[2]});
				else
					next2.add(sub);
			}
			while(queue3.size()>0){
				long[] sub = queue3.poll();
				if(dist(sub[1],sub[2],now[1],now[2])>=M&&dist(sub[3],sub[4],now[1],now[2])>=M)
					ans = Math.max(ans,sub[0]+now[0]);
				else
					next3.add(sub);
			}
			if(queue2!=null){
				next2.add(now);
				while(next2.size()>0&&next2.peek()[0]+now[0]*2<=ans)
					next2.poll();
				queue2 = new PriorityQueue<>((a,b)->Long.compare(b[0],a[0]));
				queue2.addAll(next2);
				if(queue2.size()==0)
					queue2 = null;
			}
			queue3 = new PriorityQueue<>((a,b)->Long.compare(b[0],a[0]));
			while(next3.size()>0&&next3.peek()[0]+now[0]<=ans)
				next3.poll();
			queue3.addAll(next3);
		}
		out.println((System.nanoTime()-s)/1e9);
		out.println(ans);

		out.close();
	}
	private static long dist(long a,long b,long c,long d){
...
final class SimpleWriter implements Closeable, Flushable, AutoCloseable{
	 ...
	 * ちょっとだけ速いです。
	 */
	final class SimpleOutputStream{
