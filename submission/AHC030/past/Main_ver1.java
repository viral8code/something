...
final class Main{
	...
	public static void main(String[] args){
		...
		int cycle = 2+10*(int)(eps*100);
		int len = (int)Math.ceil((1+Math.sqrt(1+4*cycle))/2);
		if(len>N*N||cycle*len-len>=N*N){
			solveDefinitively(N);
			return;
		}
		double[] value = new double[len];
		Point[] cyclePoint = new Point[len];
		ArrayList<Point> subList = new ArrayList<>();
		for(int i=0;i<len;i++){
			cyclePoint[i] = new Point(i/N,i%N);
			subList.add(cyclePoint[i]);
		}
		subList.remove(0);
		double cycleSum = 0;
		for(int i=0;i<len;i++){
			for(int j=0;j<cycle;j++){
				value[i] += question(subList);
			}
			value[i] /= cycle;
			cycleSum += value[i];
			if(i+1<len)
				subList.set(i,cyclePoint[i]);
		}
		ArrayList<Point> ans = new ArrayList<>();
		for(int i=0;i<len;i++){
			int v = (int)Math.round((cycleSum/(len-1)-value[i]));
			if(0<v)
				ans.add(cyclePoint[i]);
		}
		for(int i=len;i<N*N;i++){
			Point point = new Point(i/N,i%N);
			int v = question(point);
			if(0<v)
				ans.add(point);
		}
		int judge = answer(ans);
		for(int i=0;i<len;i++){
			value[i] = cycleSum-value[i];
		}
		TreeSet<Double> valueSet = new TreeSet<>();
		for(double v:value)
			valueSet.add(v);
		ArrayList<Double> valueList = new ArrayList<>(valueSet);
		int index = 0;
		while(judge==0){
			for(int i=ans.size()-1;i>=0;i--){
				int id = ans.get(i).x*N+ans.get(i).y;
				if(id<len){
					ans.remove(i);
				}
			}
			double c = valueList.get(index++%valueList.size());
			for(int i=0;i<len;i++){
				if(c<=value[i])
					ans.add(cyclePoint[i]);
			}
	...
	private static void solveDefinitively(int N){
	...
	private static int answer(ArrayList<Point> ans){
		if(questionCheck()){
			out.print("a "+ans.size());
			for(Point point:ans){
	...
	private static int question(ArrayList<Point> list){
		if(questionCheck()){
			out.print("q "+list.size());
	...
	private static int question(Point p){
		if(questionCheck()){
			out.println("q 1 "+p.x+" "+p.y);
			return sc.nextInt();
		}
		System.exit(0);
		return -1;
	}
	private static boolean questionCheck(){
...
final class MergeSortTree{
	...
	private MergeSortTree(MergeSortTree mst,Function<long[],long[]> mapping){
		N = mst.N;
		size = mst.size;
		node = new long[N][];
		for(int i=1;i<N;i++){
...
final class SimpleScanner{
	private final int BUFF_SIZE = 1<<20;
	...
	/**
	 * バッファからdoubleを読み込みます。
	 * オーバーフロー、アンダーフローは検知できないので注意して下さい。
	 *
	 * @return 区切り文字までの文字列をdoubleと解釈した時の値
	 */
	public double nextDouble(){
		...
		double div = 1;
		while(MathFunction.rangeCheckClose(c,'0','9')||c=='.'){
			if(c=='.'){
				decimalPoint = true;
			}
			else{
	...
	/**
	 * バッファからdouble[]を読み込みます。
	 *
	 * @param n 取得する個数
	 *
	 * @return 区切り文字で区切ってn個longを取得した結果を表すdouble[]
	 */
	...
	public double[][] nextDouble(final int n,final int m){
		final double[][] ans = new double[n][];
		for(int i = 0;i<n;++i){
			ans[i] = nextDouble(m);
		}
