...
final class Main{
	...
	private static Point[][] oilFields;
	...
	public static void main(String[] args){
		...
		for(int i=0;i<M;i++){
			int d = sc.nextInt();
			oilFields[i] = sc.nextPoint(d);
		}
		int[][] v = new int[N][N];
		final int step = 2;
		for(int[] temp:v)
			Arrays.fill(temp,M+1);
		for(int i=0;i<N;i+=step){
			for(int j=0;j<N;j+=step){
				v[i][j] = question(new Point(i,j));
			}
		}
		ArrayList<ArrayList<Point>> list = new ArrayList<>();
		dfs(0,v,new ArrayList<>(),list);
		Collections.shuffle(list);
		for(ArrayList<Point> li:list){
			int ans = answer(li);
			if(ans==1)
				return;
		}
	...
	private static void dfs(int now,int[][] v,ArrayList<Point> list,ArrayList<ArrayList<Point>> ans){
		if(now==M){
			ans.add(new ArrayList<>(new HashSet<Point>(list)));
			return;
		}
		int maxX = 0;
		int maxY = 0;
		for(Point p:oilFields[now]){
		...
		for(int i=0;i<N-maxX;i++){
			for(int j=0;j<N-maxY;j++){
				...
				for(Point p:oilFields[now]){
					v[p.x+i][p.y+j]--;
					isValid &= v[p.x+i][p.y+j]>=0;
					list.add(new Point(p.x+i,p.y+j));
				}
				if(isValid)
					dfs(now+1,v,list,ans);
				for(Point p:oilFields[now]){
					v[p.x+i][p.y+j]++;
					list.remove(list.size()-1);
				}
				if(ans.size()>=N||(System.nanoTime()-sTime)/1e6>2000)
					return;
			}
		}
	}
