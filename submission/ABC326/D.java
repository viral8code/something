...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		int N = sc.nextInt();
		char[] R = sc.nextCharArray();
		char[] C = sc.nextCharArray();
		dfs(0,new char[N][N],R,C);
		out.println("No");

		out.close();
	}
	private static final void dfs(int now,char[][] ans,char[] R,char[] C){
		int N = ans.length;
		if(now==N){
			if(check(ans,R,C)){
				out.println("Yes");
	...
	private static final boolean check(char[][] ans,char[] R,char[] C){
		int N = ans.length;
		boolean[][] checkedR = new boolean[N][3];
		boolean[][] checkedC = new boolean[N][3];
		char[][] sub = new char[N][N];
		for(int i=0;i<N;i++){
			for(int j=0;j<N;j++){
				if('A'<=ans[i][j]){
					if(checkedR[i][ans[i][j]-'A']||checkedC[j][ans[i][j]-'A'])
						return false;
					checkedR[i][ans[i][j]-'A'] = true;
		...
		for(int i=0;i<N;i++){
			for(int j=0;j<N;j++){
				...
				else if(flagR[i]&&flagC[j]){
					for(int k=0;k<3;k++){
						if(!checkedR[i][k]&&!checkedC[j][k]){
							checkedR[i][k] = true;
							checkedC[j][k] = true;
							sub[i][j] = (char)('A'+k);
							flagR[i] = true;
		...
		for(int i=0;i<N;i++)
			for(int j=0;j<3;j++)
				good &= checkedR[i][j]&&checkedC[i][j];
		if(!good)
			return false;
		for(int i=0;i<N;i++){
			for(int j=0;;j++){
