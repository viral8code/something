...
class Main{
	...
	public static void main(String[] args){
		...
		*/////////////
		...
		ArrayList<HashMap<Integer,Integer>> route = new ArrayList<>();
		for(int i=0;i<=N;i++)route.add(new HashMap<>());
		while(M-->0){
			int A = System.in.nextInt();
			int B = System.in.nextInt();
			int C = System.in.nextInt();
			route.get(A).merge(B,C,(i,j) -> Math.max(i,j));
			route.get(B).merge(A,-C,(i,j) -> Math.max(i,j));
		}
		long[] cost;
		...
		while(Q-->0){
			...
			if(memo.containsKey(X)&&memo.get(X).containsKey(Y)){
				System.out.println(memo.get(X).get(Y));
				continue;
			}
			...
			costs.add(0L);
			boolean inf = false;
			Search:while(!point.isEmpty()){
				Integer nowP = point.pollFirst();
				Long nowC = costs.pollFirst();
				for(Integer next:route.get(nowP).keySet()){
					if(list.containsKey(next)){
						int sub = list.get(next).compareTo(nowC+route.get(nowP).get(next));
						if(sub<0){
							inf = true;
							break Search;
						}else if(sub>0){
							list.put(next,nowC+route.get(nowP).get(next));
							point.add(next);
							costs.add(nowC+route.get(nowP).get(next));
						}
					}else{
						list.put(next,nowC+route.get(nowP).get(next));
						point.add(next);
			...
			else if(!list.containsKey(Y))
				System.out.println("nan");
			else{
				if(!memo.containsKey(X))
					memo.put(X,new HashMap<>());
				memo.get(X).putAll(list);
