...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		ArrayList<TreeSet<Integer>> par = new ArrayList<>();
		for(int i=0;i<=N;i++){
			set.add(new TreeSet<>());
			par.add(new TreeSet<>());
		}
		...
		for(int i=1;i<=N;i++){
			...
			for(Integer next:set.get(i))
				bfs.add(next);
			while(bfs.size()>0){
				int now = bfs.pollFirst();
				for(Integer next:set.get(now)){
					if(set.get(i).contains(next))
						continue;
					set.get(i).add(next);
					par.get(next).add(i);
				}
