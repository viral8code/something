...
final class Main{
	...
	public static void main(String[] args){
		...
		for(int i=0;i<N;i++)
			queue.add(new int[]{W[i],i});
		for(int i=0;i<N;i++)
			index[i] = queue.poll()[1];
		for(int i=0;i<N;i++){
			dp[i] = new long[W[index[i]]];
			boolean trigger = true;
			for(int j=0;W[index[j]]<W[index[i]];j++){
				if(!graph[index[i]][index[j]])
					continue;
				trigger = false;
				for(int k=W[index[i]]-1;k>=W[index[j]];k--){
					dp[i][k] = Math.max(dp[i][k],dp[i][k-W[index[j]]]+dp[j][W[index[j]]-1]);
				}
			}
			dp[i][W[index[i]]-1] = Math.max(dp[i][W[index[i]]-1],1);
		}
		long ans = 0;
		for(int i=0;i<N;i++){
			ans += dp[i][W[index[i]]-1]*A[i];
		}
		out.println(ans);
