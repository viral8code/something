...
final class Main{
	...
	public static void main(String[] args){
		...
		int M = sc.nextInt();
		long K = sc.nextLong();
		long lcm = MathFunction.lcm(N,M);
		long count = lcm/M+lcm/N-2;
		long L = K/count;
		K %= count;
		if(K==0){
			System.out.println(Math.max((lcm/M-1)*M,(lcm/N-1)*N)+lcm*(L-1));
			return;
		}
		if(N>M){
			N ^= M;
		...
		if(ans+N*ans/M==K)
			out.println(ans*N+lcm*L);
		else
			out.println((N*ans/M+1)*M+lcm*L);
