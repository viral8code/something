...
class Main{
	...
	public static void main(String[] args)throws IOException{
		...
		int E = System.in.nextInt();
		int[][] route = new int[E][2];
		for(int i=0;i<E;i++){
			int U = Math.min(N+1,System.in.nextInt());
			int V = Math.min(N+1,System.in.nextInt());
			route[i][0] = U;
			route[i][1] = V;
		}
		boolean[] route_break = new boolean[E];
		ArrayDeque<Integer> list = new ArrayDeque<Integer>();
		int Q = System.in.nextInt();
		for(int i=0;i<Q;i++){
			int temp = System.in.nextInt()-1;
			route_break[temp] = true;
			list.addFirst(temp);
		}
		ArrayList<ArrayDeque<Integer>> connected = new ArrayList<ArrayDeque<Integer>>(N+2);
		for(int i=0;i<N+2;i++)connected.add(new ArrayDeque<Integer>());
		for(int i=0;i<E;i++){
			if(route_break[i])continue;
			if(route[i][0]==N+1)continue;
			connected.get(route[i][0]).add(route[i][1]);
			connected.get(route[i][1]).add(route[i][0]);
		}
		boolean[] bright = new boolean[N+2];
		boolean[] checked = new boolean[N+2];
		int[] number = new int[N+2];
		ArrayDeque<Integer> bfs = new ArrayDeque<Integer>();
		ArrayList<ArrayDeque<Integer>> group = new ArrayList<ArrayDeque<Integer>>();
		int count = 0;
		for(int i=1;i<N+2;i++){
		...
		int answer = -1;
		for(int temp:group.get(number[N+1])){
			answer++;
			bright[temp] = true;
		}
		...
		while(Q>0){
			...
			int V = route[temp][1];
			if(U==N+1){
			}else if(bright[U]&&!bright[V]){
				for(int city:group.get(number[V])){
					answer++;
					bright[city] = true;
				}
			}else if(!bright[U]&&bright[V]){
				for(int city:group.get(number[U])){
					answer++;
					bright[city] = true;
				}
			}else if(number[U]!=number[V]&&!bright[U]&&!bright[V]){
				group.get(number[U]).addAll(group.get(number[V]));
				for(int tmp:group.get(number[V]))number[tmp] = number[U];
			}
			answers[--Q] = answer;
		}
