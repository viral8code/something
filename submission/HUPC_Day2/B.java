...
class Main{
	public static void main(String[]args){
		...
		HashMap<Integer,Long> invId= new HashMap<>();
		int index = 0;
		...
		for(int i=0;i<N;i++){
			...
			for(int j=0;j<id.size();j++){
				for(int k=0;k<id.size();k++){
					if(invId.get(k)<L||R<invId.get(k))
						continue;
					int next = id.get( MathFunction.lcm( invId.get(j), invId.get(k) ) );
					dp[i+1][next] += dp[i][j];
					dp[i+1][next] %= mod;
				}
			}
		}
		out.println(dp[N][id.get(X)]);
		out.close();
	}
}
class Factorial{
