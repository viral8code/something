...
final class Main {
	...
	private static void make(int[] A,int[] B,int K,int now){
		if(now==A.length)
			return;
		...
		if(K<=small.size()){
		...
		else if(K>(A.length-now)*(A.length-now-1)/2+small.size()+now+1){
			Collections.sort(big);
			//out.println("big");
			K -= (A.length-now)*(A.length-now-1)/2+small.size()+now+1;
			//out.println(K);
			int index = -1;
			if(big.size()==0){
				System.arraycopy(A,now,B,now,B.length-now);
				return;
			}
			for(int i=now;i<A.length;i++)
				if(A[i]==big.get(K-1))
			...
			//out.println("loop(K="+(K-small.size())+")");
			B[now] = A[now];
			make(A,B,K-small.size(),now+1);
		}
	}
}
