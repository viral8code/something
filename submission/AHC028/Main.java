...
final class Main{
	...
	private static final long TIME_LIMIT = 1_900_000_000L;
	private static final Random rm = new Random();
	private static final HashMap<Character,ArrayList<Point>> map = new HashMap<>();

	static final class Node{
		final int dist;
		final ArrayList<Point> answer;
		public Node(final int d,final ArrayList<Point> a){
			dist = d;
			answer = a;
		}
	}
	...
	public static void main(String[] args){
		...
		final int si = sc.nextInt();
		final int sj = sc.nextInt();
		for(char c='A';c<='Z';c++)
			map.put(c,new ArrayList<>());
		for(int i=0;i<N;i++){
			final String A = sc.next();
			for(int j=0;j<N;j++)
				map.get(A.charAt(j)).add(new Point(i,j));
		}
		final String[] t = sc.next(M);
		Node answer = new Node(Integer.MAX_VALUE,null);
		while(System.nanoTime()-startTime<TIME_LIMIT){
			final String str = makeString(t);
			final Node node = getNode(str,si,sj);
			if(node.dist<answer.dist)
				answer = node;
		}
		for(final Point p:answer.answer)
			out.printf("%d %d\n",p.x,p.y);

		out.close();
	}

	private static Node getNode(final String ans,final int x,final int y){
		PriorityQueue<Node> queue = new PriorityQueue<>((a,b)->Integer.compare(b.dist,a.dist));
		PriorityQueue<Node> next = new PriorityQueue<>((a,b)->Integer.compare(b.dist,a.dist));
		for(final Point p:map.get(ans.charAt(0))){
			final ArrayList<Point> temp = new ArrayList<>();
			temp.add(p);
			queue.add(new Node(Math.abs(x-p.x)+Math.abs(y-p.y),temp));
		}
		while(queue.size()>size)
			queue.poll();
		for(int i=1;i<ans.length();i++){
			for(final Node now:queue){
				final Point bef = now.answer.get(i-1);
				for(final Point p:map.get(ans.charAt(i))){
					final ArrayList<Point> temp = new ArrayList<Point>(now.answer);
					temp.add(p);
					next.add(new Node(now.dist+Math.abs(bef.x-p.x)+Math.abs(bef.y-p.y),temp));
				}
	...
	private static String makeString(final String[] t){
		final StringBuilder answer = new StringBuilder();
		final int first = rm.nextInt(t.length);
		...
		while(list.size()>0){
			...
			for(int j=0;j<list.size();j++){
				int sub = list.get(j).length();
				while(!answer.substring(answer.length()-sub).equals(list.get(j).substring(0,sub)))
					sub--;
				if(len<sub){
					next = j;
					len = sub;
				}
				else if(len==sub&&rm.nextInt(2)==0){
					next = j;
					len = sub;
				}
			}
			answer.append(list.get(next).substring(len));
			list.remove(next);
		}
...
@SuppressWarnings("unchecked")
abstract class LongLazySegmentTree<F>{
	private final int size, log;
	 ...
	 * @param defaultF 恒等写像
	 */
	public LongLazySegmentTree(final int N,final long defaultS,final F defaultF){
		this(N,defaultS,defaultF,false);
	}
	 ...
	 * @param is1indexed 各メソッドに渡すインデックスは1-indexedか
	 */
	public LongLazySegmentTree(final int N,final long defaultS,final F defaultF,final boolean is1indexed){
		this.log = 32-Integer.numberOfLeadingZeros(N-1);
		this.size = (1<<log)-(is1indexed?1:0);
		this.defaultS = defaultS;
		this.defaultF = defaultF;
		this.node = new long[1<<log+1];
		this.lazy = (F[])new Object[1<<log+1];
		clear();
	}
	 ...
	 * @param is1indexed 各メソッドに渡すインデックスは1-indexedか
	 */
	public LongLazySegmentTree(final long[] defaultValues,final long defaultS,final F defaultF,final boolean is1indexed){
		this(defaultValues.length,defaultS,defaultF,is1indexed);
		updateAll(defaultValues);
	}
	 ...
	 * @param defaultValues 元となる配列
	 */
	private void updateAll(final long[] defaultValues){
		System.arraycopy(defaultValues,0,node,1<<log,defaultValues.length);
		for(int i = (1<<log)-1;i>0;i--){
			node[i] = function(node[i<<1],node[i<<1|1]);
		}
	 ...
	 * @param x 上書きする値
	 */
	public void update(int index,final long x){
		index += size;
		spreadLine(index);
		node[index] = x;
		update(index);
	 ...
	 * @return 要素に関数を適用した結果
	 */
	abstract public long function(long s1,long s2);
	 ...
	 * @return 要素に演算を適用した結果
	 */
	abstract public long mapping(long s,F f);
}
