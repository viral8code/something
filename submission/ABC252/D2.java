...
class Main{
	...
	public static void main(String[] args){
		int N = readI();
		int[] A = new int[N];
		int[] Asub = newint[200001];
		ArrayList<Integer> Asubsub = new ArrayList<Integer>();
		for(int i=0;i<N;i++){
			A[i] = readI();
			if(Asub[A[i]]>0){
				Asubsub.add(A[i]);
			}
			Asub.set(A[i],Asub.get(A[i])+1);
		}
		Asubsubs = new HashSet<Integer>(Asubsub);
		for(int i=0;i<N-3;i++){
			int sum = (N-i-1)*(N-i-2)*(N-i-3)/6;
			Asub.set(A[i],Asub.get(A[i])-1);
			if(Asubsub.size()>0){
				ArrayList<Integer> temp = new ArrayList<Integer>(Asubsubs);
