...
class Main{
	...
	public static void main(String[] args){
		...
		ArrayList<Integer> chohuku = new ArrayList<Integer>();
		long ans = 0;
		for(int i=0;i<A.size();i++){
			if(A.get(i).size()>0){
				chohuku.add(A.get(i).size());
				long temp = N - A.get(i).size();
				ans += Math.max((temp*(temp-1)*(temp-2))/6,0);
			}
		}
		for(int i=0;i<chohuku.size();i++){
			long temp = 1;
			for(int j=2;j<chohuku.get(i);j++) temp *= j;
			ans /= temp;
		}
		retE(ans);
	}
