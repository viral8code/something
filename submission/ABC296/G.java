...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		while(index<Q&&query[index][0]<=maxX){
			...
			if(query[index][0]<maxX&&0<s&&t<0)
				ans[query[index++][2]] = "IN";
			else if(query[index][0]==maxX&&point[lowerIndex][1]<=query[index][1]&&query[index][1]<=point[upperIndex][1]||s==0&&check(u1,u2)||t==0&&check(v1,v2))
