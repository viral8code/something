...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		while(T-->0){
			...
			String S = "A"+sc.next();
			char[] ans = new char[N];
			boolean flag = make(true,1,graph,S,ans,new boolean[N+1]);
			if(!flag)
				flag = make(false,1,graph,S,ans,new boolean[N+1]);
			if(flag){
				out.println("Yes");
				out.println(ans);
			}
	...
	private static boolean make(boolean toB,int now,int[][] graph,String S,char[] ans,boolean[] isChecked){
		ans[now-1] = toB?'B':'W';
		int count = 0;
		isChecked[now] = true;
		for(int next:graph[now]){
			if(isChecked[next])
				count += S.charAt(now)==ans[next-1]?1:-1;
			else
				count += make(S.charAt(now)=='B',next,graph,S,ans,isChecked)?1:-1;
		}
		if(count<0)
			ans[now-1] = !toB?'B':'W';
		out.println(now+" "+(count>0));
		return count>0;
	}
}
/*
