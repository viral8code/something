...
final class Main {
	...
	public static void main ( String[] args ) {
		...
		while(T-->0){
			int N = sc.nextInt();
			int D = sc.nextInt();
			int[][] graph = sc.nextGraph(N,N*D);
			out.println(judge(N,D,graph)?"Yes":"No");
		}

		out.close();
	}
	private static boolean judge(int N,int D,int[][] graph){
		int limit = D*2+1;
		if(limit==N)
			return true;
		for(int i=1;i<=N;i++){
			...
			set.add(i);
			if(D<=dfs(i,limit-1,graph,set))
				return false;
		}
		return true;
	}
	private static int dfs(int now,int limit,int[][] graph,HashSet<Integer> set){
		if(limit==0){
			int count = 0;
			for(int p:set)
				for(int p2:graph[p])
		...
		for(int next:graph[now]){
			if(set.contains(next))
				continue;
			set.add(next);
			ans = Math.max(ans,dfs(next,limit-1,graph,set));
			set.remove(next);
		}
		return ans;
	}
